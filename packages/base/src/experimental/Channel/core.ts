...
import type { Channel } from './primitives'
  ...
  const collector: Channel<Env, OutErr, OutElem, OutDone, OutErr, OutElem2, OutDone> = readWith(
    (o) =>
      O.match_(
        f(o),
        () => collector,
        (out2) => zipRight_(write(out2), collector)
      ),
    (e) => fail(e),
    (z) => end(z)
  )
...
function contramapReader<InErr, InElem, InDone0, InDone>(
  ...
  return readWith(
    (_in) => zipRight_(write(_in), contramapReader(f)),
    (err) => fail(err),
    (done) => end(f(done))
  )
}
...
function contramapInReader<InErr, InElem0, InElem, InDone>(
  ...
  return readWith(
    (_in) => zipRight_(write(f(_in)), contramapInReader(f)),
    (err) => fail(err),
    (done) => end(done)
  )
}
...
function contramapMReader<Env1, InErr, InElem, InDone0, InDone>(
  ...
  return readWith(
    (_in) => zipRight_(write(_in), contramapMReader(f)),
    (err) => fail(err),
    ...
    M.map(({ input, pullL, pullR }) => {
      ...
      > =>
        Ex.match_(
            ...
            if (result._mergeDecisionTag === MD.MergeDecisionTag.Done) {
              ...
              return I.map_(
                ...
                Ex.match(
                  ...
                  (elem) => zipRight_(write(elem), go(single(result.f)))
                )
              )
            }
          ...
          (elem) => I.map_(I.fork(pull), (leftFiber) => zipRight_(write(elem), go(both(leftFiber, fiber))))
        )
          ...
          return unwrap(
            I.map_(
              ...
              Ex.match(
                ...
                (elem) => zipRight_(write(elem), go(new MS.LeftDone(state.f)))
              )
            )
          )
        } else {
          return unwrap(
            I.map_(
              ...
              Ex.match(
                ...
                (elem) => zipRight_(write(elem), go(new MS.RightDone(state.f)))
              )
            )
          )
        }
...
const mapOutMReader = <Env, Env1, OutErr, OutErr1, OutElem, OutElem1, OutDone>(
  ...
  readWith(
    (out) => bind_(fromEffect(f(out)), (_) => zipRight_(write(_), mapOutMReader(f))),
    (e) => fail(e),
    (z) => end(z)
  )
  ...
  return unwrap(
    Ref.modify_(ref, (v) => {
      if (isEmpty(v)) {
        return tuple(
          readWith(
            (_in) => zipRight_(write(_in), buffer(empty, isEmpty, ref)),
            (err) => fail(err),
            (done) => end(done)
          ),
          v
        ...
        return tuple(zipRight_(write(v), buffer(empty, isEmpty, ref)), empty)
      }
    })
  )
}
...
export function id<Err, Elem, Done>(): Channel<unknown, Err, Elem, Done, Err, Elem, Done> {
  return readWith(
    (_in) => zipRight_(write(_in), id<Err, Elem, Done>()),
    (err) => fail(err),
    (done) => end(done)
  )
}
...
export function fromInput<Err, Elem, Done>(
  ...
  return unwrap(
    input.takeWith(
      (_) => halt(_),
      (_) => zipRight_(write(_), fromInput(input)),
      (_) => end(_)
    )
  )
}
...
export function fromQueue<Err, Elem, Done>(
  ...
  return bind_(
    ...
    Ex.match(
      ...
      (elem) => zipRight_(write(elem), fromQueue(queue))
    )
  )
}
...
export function toQueue<Err, Done, Elem>(
  ...
  return readWithCause(
    (in_: Elem) => zipRight_(fromEffect(Q.offer_(queue, Ex.succeed(in_))), toQueue(queue)),
...
export function writeAll<Out>(
  ...
  return AR.foldr_(
    ...
    (out, conduit) => zipRight_(write(out), conduit)
  )
}
