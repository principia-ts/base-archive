...
function maybeCloseBoth<Env>(
  ...
  if (l && r) return pipe(I.result(l), I.crossWith(I.result(r), Ex.crossRight_))
    ...
    while (!L.isEmpty(conts)) {
      ...
      if (head._tag === C.ChannelTag.ContinuationK) {
        ...
        return pipe(
          ...
          I.chain((finExit) => this.unwindAllFinalizers(Ex.crossRight_(acc, finExit), L.tail(conts), exit))
        )
      }
    }
    return I.done(acc)
  ...
  close(exit: Exit<unknown, unknown>): IO<Env, never, unknown> | undefined {
    ...
    if (this.subexecutorStack) {
      if (this.subexecutorStack._tag === SubexecutorStackTag.Inner) {
        ...
        if (fin1 && fin2) {
          closeSubexecutors = pipe(I.result(fin1), I.crossWith(I.result(fin2), Ex.crossRight_))
        } else if (fin1) {
    ...
    if (closeSubexecutors || runInProgressFinalizer || closeSelf) {
      return pipe(
        ...
        I.map(([a, b, c]) => pipe(a, Ex.crossRight(b), Ex.crossRight(c))),
        I.uninterruptible
      )
    }
  }
