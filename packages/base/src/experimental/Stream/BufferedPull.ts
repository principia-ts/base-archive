...
export function update<R, E, A>(self: BufferedPull<R, E, A>): I.IO<R, O.Option<E>, void> {
  return ifNotDone_(
    ...
    I.matchIO_(
      self.upstream,
      O.match(
        () => I.crossRight_(Ref.set_(self.done, true), Pull.end),
        (e) => Pull.fail(e)
      ),
  ...
  return ifNotDone_(
    ...
    pipe(
      ...
      Ref.modify(([chunk, idx]) => {
        if (idx >= chunk.length) {
          return tuple(I.crossRight_(update(self), pullElement(self)), tuple(C.empty<A>(), 0))
        } else {
...
export function pullChunk<R, E, A>(self: BufferedPull<R, E, A>): I.IO<R, O.Option<E>, C.Chunk<A>> {
  return ifNotDone_(
    ...
    pipe(
      ...
      Ref.modify(([chunk, idx]) => {
        if (idx >= chunk.length) {
          return tuple(I.crossRight_(update(self), pullChunk(self)), tuple(C.empty<A>(), 0))
        } else {
