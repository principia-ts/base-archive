...
export function getEitherT<F extends HKT.URIS, C = HKT.Auto>(
  M: P.Monad<F, C>
): P.Monad<EitherTURI<F>, V<C>>;
export function getEitherT<F>(M: P.Monad<HKT.UHKT<F>>): P.Monad<EitherTURI<HKT.UHKT<F>>, E.V> {
  ...
  return HKT.instance<EitherT<HKT.UHKT<F>>>({
    ...
    flatten: <E, A, D>(mma: HKT.HKT<F, E.Either<E, HKT.HKT<F, E.Either<D, A>>>>) =>
      pipe(
        mma,
        M.map(E.fold((e) => M.pure(E.widenE<D>()(E.left(e))), M.map(E.widenE<E>()))),
        M.flatten
      ),
    unit: () => M.pure(E.unit()),
    absolve: <E, E1, A>(fa: HKT.HKT<F, E.Either<E, E.Either<E1, A>>>) =>
      M.map_(fa, E.fold(E.left, identity)),
