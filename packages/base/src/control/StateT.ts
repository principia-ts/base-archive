...
export type StateTURI<F extends HKT.URIS> = HKT.PrependURI<
  StateInURI,
  HKT.AppendURI<F, StateOutURI>
>;

export interface StateT<M extends HKT.URIS, C = HKT.Auto>
  extends P.MonadState<StateTURI<M>, V<C>> {}

export function getMonadStateT<F extends HKT.URIS, C>(
  M: P.Monad<F, C>
): P.MonadState<StateTURI<F>, V<C>>;
export function getMonadStateT<F>(
  M: P.Monad<HKT.UHKT<F>>
): P.MonadState<StateTURI<HKT.UHKT<F>>, V<HKT.Auto>> {
  const map_: P.MapFn_<StateTURI<HKT.UHKT<F>>, V<HKT.Auto>> = (fa, f) => (s) =>
  ...
  const ap_: P.ApFn_<StateTURI<HKT.UHKT<F>>, V<HKT.Auto>> = (fab, fa) =>
