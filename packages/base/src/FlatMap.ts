...
export interface FlatMap<F extends HKT.URIS, TC = HKT.Auto> extends HKT.Base<F, TC> {
  readonly flatMap_: FlatMapFn_<F, TC>
  readonly flatMap: FlatMapFn<F, TC>
  readonly flatten: FlattenFn<F, TC>
}

export interface FlatMapFn<F extends HKT.URIS, TC = HKT.Auto> {
...
export interface FlatMapFn_<F extends HKT.URIS, TC = HKT.Auto> {
...
export interface FlatMapFnComposition<F extends HKT.URIS, G extends HKT.URIS, TCF = HKT.Auto, TCG = HKT.Auto> {
  <
    NF1 extends string,
    KF1,
    QF1,
...
export interface FlatMapFnComposition_<F extends HKT.URIS, G extends HKT.URIS, TCF = HKT.Auto, TCG = HKT.Auto> {
  <
    NF extends string,
    KF,
    QF,
...
export function flatMapF<F extends HKT.URIS, TC = HKT.Auto>(F: Monad<F, TC>): FlatMapFn<F, TC>
export function flatMapF<F>(F: Monad<HKT.UHKT<F>>): FlatMapFn<HKT.UHKT<F>> {
  return (f) => (ma) => F.flatten(F.map_(ma, f))
}

export function flatMapF_<F extends HKT.URIS, TC = HKT.Auto>(F: Monad<F, TC>): FlatMapFn_<F, TC>
export function flatMapF_<F>(F: Monad<HKT.UHKT<F>>): FlatMapFn_<HKT.UHKT<F>> {
...
export function tapF<F>(F: Monad<HKT.UHKT<F>>): TapFn<HKT.UHKT<F>> {
  return (f) => flatMapF(F)((a) => F.map_(f(a), () => a))
}
...
export function tapF_<F>(F: Monad<HKT.UHKT<F>>): TapFn_<HKT.UHKT<F>> {
  return (ma, f) => flatMapF_(F)(ma, (a) => F.map_(f(a), () => a))
}
