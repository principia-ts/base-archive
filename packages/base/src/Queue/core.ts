...
export function takeBetween(min: number, max: number) {
  return <RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>): I.IO<RB, EB, Chunk<B>> => {
 ...
 * If the queue is already shutdown, the `IO` will resume right away.
 */
export function awaitShutdown<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>) {
 ...
 * How many elements can hold in the queue
 */
export function capacity<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>) {
 ...
 * `true` if `shutdown` has been called.
 */
export function isShutdown<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>) {
 ...
 * Places one value in the queue.
 */
export function offer<A>(a: A): <RA, RB, EA, EB, B>(self: Queue<RA, RB, EA, EB, A, B>) => I.IO<RA, EA, boolean> {
 ...
 * Places one value in the queue.
 */
export function offer_<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>, a: A): I.IO<RA, EA, boolean> {
  return self.offer(a)
}
...
export function offerAll<A>(
  as: Iterable<A>
): <RA, RB, EA, EB, B>(self: Queue<RA, RB, EA, EB, A, B>) => I.IO<RA, EA, boolean> {
 ...
 *
 */
export function offerAll_<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>, as: Iterable<A>) {
 ...
 * Future calls to `offer*` and `take*` will be interrupted immediately.
 */
export function shutdown<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>) {
 ...
 * elements to be added to the queue.
 */
export function size<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>) {
  return self.size
}
 ...
 * return a computation that resumes when an item has been added to the queue.
 */
export function take<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>) {
  return self.take
}
 ...
 * is empty returns empty list.
 */
export function takeAll<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>) {
  return self.takeAll
}
...
export function takeAllUpTo(
  n: number
): <RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>) => I.IO<RB, EB, Chunk<B>> {
 ...
 * Takes up to max number of values in the queue.
 */
export function takeAllUpTo_<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>, n: number) {
...
export function zipWith<RA1, RB1, EA1, EB1, A1 extends A, C, B, D, A>(
  that: Queue<RA1, RB1, EA1, EB1, A1, C>,
  f: (b: B, c: C) => D
): <RA, RB, EA, EB>(self: Queue<RA, RB, EA, EB, A, B>) => Queue<RA & RA1, RB & RB1, EA1 | EA, EB1 | EB, A1, D> {
...
export function dimapM<A, B, C, RC, EC, RD, ED, D>(
  f: (c: C) => I.IO<RC, EC, A>,
  g: (b: B) => I.IO<RD, ED, D>
): <RA, RB, EA, EB>(self: Queue<RA, RB, EA, EB, A, B>) => Queue<RC & RA, RD & RB, EC | EA, ED | EB, C, D> {
...
export function contramapM<C, RA2, EA2, A>(f: (c: C) => I.IO<RA2, EA2, A>) {
  return <RA, RB, EA, EB, B>(self: Queue<RA, RB, EA, EB, A, B>) => dimapM_(self, f, I.pure)
}
...
export function contramap<C, A>(
  f: (c: C) => A
): <RA, RB, EA, EB, B>(self: Queue<RA, RB, EA, EB, A, B>) => Queue<RA, RB, EA, EB, C, B> {
...
): <RA, RB, EA, EB, B>(self: Queue<RA, RB, EA, EB, A, B>) => Queue<RA & R2, RB, E2 | EA, EB, A1, B> {
...
export function filterInput<A, A1 extends A>(
  f: (_: A1) => boolean
): <RA, RB, EA, EB, B>(self: Queue<RA, RB, EA, EB, A, B>) => Queue<RA, RB, EA, EB, A1, B> {
...
export function filterOutputM_<RA, RB, EA, EB, A, B, RB1, EB1>(
  ...
  return new (class extends Queue<RA, RB & RB1, EA, EB | EB1, A, B> {
    ...
    offer                                     = (a: A): IO<RA, EA, boolean> => self.offer(a)
    offerAll                                  = (as: Iterable<A>): IO<RA, EA, boolean> => self.offerAll(as)
    shutdown: UIO<void>                       = self.shutdown
    size: UIO<number>                         = self.size
    take: IO<RB & RB1, EB1 | EB, B>           = I.bind_(self.take, (b) => I.bind_(f(b), (p) => (p ? I.succeed(b) : this.take)))
...
export function filterOutputM<B, RB1, EB1>(
  f: (b: B) => IO<RB1, EB1, boolean>
): <RA, RB, EA, EB, A>(self: Queue<RA, RB, EA, EB, A, B>) => Queue<RA, RB & RB1, EA, EB | EB1, A, B> {
...
export function mapM<B, R2, E2, C>(f: (b: B) => I.IO<R2, E2, C>) {
  return <RA, RB, EA, EB, A>(self: Queue<RA, RB, EA, EB, A, B>) => dimapM_(self, (a: A) => I.pure(a), f)
}
...
export function map<B, C>(
  f: (b: B) => C
): <RA, RB, EA, EB, A>(self: Queue<RA, RB, EA, EB, A, B>) => Queue<RA, RB, EA, EB, A, C> {
 ...
 */
export function poll<RA, RB, EA, EB, A, B>(self: Queue<RA, RB, EA, EB, A, B>): IO<RB, EB, O.Option<B>> {
