...
export function both<R, R1>(
  left: BlockedRequests<R>,
  right: BlockedRequests<R1>
      ...
      I.foreachUnit((requestsByDataSource) =>
        I.foreachUnitPar_(requestsByDataSource.toIterable, ([dataSource, sequential]) =>
          I.gen(function* (_) {
            ...
            yield* _(
              I.foreachUnit_(blockedRequests, (br) =>
...
function flatten<R>(blockedRequests: BlockedRequests<R>): List<Sequential<R>> {
  const go = <R>(
    brs: List<BlockedRequests<R>>,
    flattened: List<Sequential<R>>
  ): Ev.Eval<List<Sequential<R>>> =>
    Ev.gen(function* (_) {
      ...
      if (L.isEmpty(sequential)) return L.reverse(updated);
      else return yield* _(go(sequential, updated));
    });
  ...
  const go = <R>(
    ...
    Ev.gen(function* (_) {
      switch (blockedRequests._tag) {
        case "Empty": {
          if (L.isEmpty(stack)) return [parallel, sequential];
          else
            return yield* _(
              go(L.unsafeFirst(stack) as BlockedRequests<R>, L.tail(stack), parallel, sequential)
            );
        }
        case "Then": {
            ...
            case "Single": {
              return yield* _(
                go(
                  ...
                  stack,
                  parallel,
                  L.append(blockedRequests.right)(sequential)
                )
              );
            }
          }
        ...
        case "Both": {
          return yield* _(
            go(blockedRequests.left, L.append(blockedRequests.right)(stack), parallel, sequential)
          );
        }
        case "Single": {
          if (L.isEmpty(stack))
            ...
            return yield* _(
              go(
                ...
                L.tail(stack),
                parallel["++"](
                  Par.from(blockedRequests.dataSource, blockedRequests.blockedRequest)
                ),
                sequential
              )
            );
...
function merge<R>(sequential: List<Sequential<R>>, parallel: Par.Parallel<R>): List<Sequential<R>> {
  if (L.isEmpty(sequential)) {
    ...
    getIterableSize(L.unsafeFirst(sequential)?.keys) === 1 &&
    getIterableSize(parallel.keys) === 1
  ) {
    return pipe(
