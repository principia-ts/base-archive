...
export type UHKTKleisliCategory<F> = [URI<"HKTKleisliCategory", CustomType<"F", F>>];

export interface URItoKleisliCategory {}

export interface AlternativeURI {
   KleisliCategory: URItoKleisliCategory;
}
...
> {
   ...
   ["HKTKleisliCategory"]: HKT2<AccessCustom<FC, "F">, I, A>;
}
...
export type Index<U extends ConcreteURIS, TC> = TC extends { [u in U]: infer X } ? (X extends ConcreteURIS ? X : U) : U;

export type Kind<F extends URIS, C, N extends string, K, Q, W, X, I, S, R, E, A, TC = {}> = ((...x: F) => any) extends (
   fst: infer XURI,
   ...rest: infer Rest
) => any
