...
export interface MapBothFnComposition<F extends HKT.URIS, G extends HKT.URIS, TCF = HKT.Auto, TCG = HKT.Auto> {
   <
      A,
      NF1 extends string,
      KF1,
...
export interface MapBothFnComposition_<F extends HKT.URIS, G extends HKT.URIS, TCF = HKT.Auto, TCG = HKT.Auto> {
   <
      NF extends string,
      KF,
      QF,
...
export function mapBothF<F extends HKT.URIS, TC = HKT.Auto>(F: Applicative<F, TC>): MapBothFn<F, TC>;
export function mapBothF<F>(F: Applicative<HKT.UHKT<F>>): MapBothFn<HKT.UHKT<F>> {
   return (fb, f) => (fa) => F.map_(F.both_(fa, fb), ([a, b]) => f(a, b));
}

export function mapBothF_<F extends HKT.URIS, TC = HKT.Auto>(F: Applicative<F, TC>): MapBothFn_<F, TC>;
