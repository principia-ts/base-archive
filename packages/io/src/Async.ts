...
export class FoldInstruction<R, E, A, R1, E1, B, R2, E2, C> extends Async<
  R & R1 & R2,
  E1 | E2,
  B | C
> {
...
export function flatMap_<R, E, A, Q, D, B>(
  ma: Async<R, E, A>,
  f: (a: A) => Async<Q, D, B>
): Async<Q & R, D | E, B> {
...
export function tap<A, Q, D, B>(
  f: (a: A) => Async<Q, D, B>
): <R, E>(ma: Async<R, E, A>) => Async<Q & R, D | E, A> {
  return (ma) => tap_(ma, f);
}
  ...
  async wait(): Promise<Ex.AsyncExit<any, any>[]> {
    const t = await Promise.all(
      Array.from(this.running).map((p) =>
        p.then((a) => Ex.success(a)).catch((e) => Promise.resolve(e))
      )
    );
...
export const AsynctoIO = new AtomicReference<
  O.Option<<R, E, A>(_: Async<R, E, A>) => I.IO<R, E, A>>
>(O.none());
