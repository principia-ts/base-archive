...
export function asksServices<SS extends Record<string, Tag<any>>>(
  ...
  f: (a: { [k in keyof SS]: [SS[k]] extends [Tag<infer T>] ? T : unknown }) => B
) => I.URIO<UnionToIntersection<{ [k in keyof SS]: [SS[k]] extends [Tag<infer T>] ? Has<T> : unknown }[keyof SS]>, B> {
  return (f) =>
    I.asks((r: UnionToIntersection<{ [k in keyof SS]: [SS[k]] extends [Tag<infer T>] ? Has<T> : unknown }[keyof SS]>) =>
      f(R.map_(s, (v) => r[v.key]) as any)
    )
}
 ...
 */
export function asksServiceM<T>(s: Tag<T>): <R, E, B>(f: (a: T) => I.IO<R, E, B>) => I.IO<R & Has<T>, E, B> {
...
export function asksServiceF<T>(
  s: Tag<T>
): <K extends keyof T & { [k in keyof T]: T[k] extends (...args: any[]) => I.IO<any, any, any> ? k : never }[keyof T]>(
  k: K
) => (
  ...args: T[K] extends (...args: infer ARGS) => I.IO<any, any, any> ? ARGS : unknown[]
) => T[K] extends (...args: any[]) => I.IO<infer R, infer E, infer A> ? I.IO<R & Has<T>, E, A> : unknown[] {
...
export function giveServiceM<T>(_: Tag<T>) {
  return <R, E>(f: I.IO<R, E, T>) => <R1, E1, A1>(ma: I.IO<R1 & Has<T>, E1, A1>): I.IO<R & R1, E | E1, A1> =>
 ...
 * Provides the service with the required Service Entry, depends on global HasRegistry
 */
export function giveService<T>(_: Tag<T>): (f: T) => <R1, E1, A1>(ma: I.IO<R1 & Has<T>, E1, A1>) => I.IO<R1, E1, A1> {
...
export function asksRegion<K, T>(h: Tag<Region<T, K>>): <A>(e: (_: T) => A) => I.IO<Has<Region<T, K>>, never, A> {
...
export function asksServiceIn<A>(
  _: Tag<A>
): <K, T>(h: Tag<Region<Has<A> & T, K>>) => <B>(f: (_: A) => B) => I.IO<Has<Region<Has<A> & T, K>>, never, B> {
  return (h) => (f) =>
    useRegion(h)(
      asksServiceM(_)((a) =>
        pipe(
