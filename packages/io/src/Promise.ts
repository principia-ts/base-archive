...
export class Promise<E, A> {
   ...
   */
  complete: (effect: FIO<E, A>) => I.UIO<boolean> = to(this)
   ...
   * fibers waiting on the value of the promise.
   */
  die = (e: unknown): I.UIO<boolean> => this.completeWith(I.die(e))
  ...
  interrupt = (): I.UIO<boolean> =>
    pipe(
      fiberId(),
      I.flatMap((id) => this.completeWith(interruptAsIO(id)))
    )
   ...
   */
  succeed = (a: A): I.UIO<boolean> => this.completeWith(I.succeed(a))
...
export function to<E, A>(p: Promise<E, A>) {
  return <R>(effect: I.IO<R, E, A>): I.IO<R, never, boolean> =>
    uninterruptibleMask(({ restore }) => I.flatMap_(I.result(restore(effect)), (x) => p.done(x)))
}
...
export function make<E, A>() {
  return I.flatMap_(fiberId(), (id) => makeAs<E, A>(id))
}
