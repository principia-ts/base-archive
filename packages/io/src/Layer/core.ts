  ...
  switch (_I._tag) {
    ...
    case LayerInstructionTag.Chain: {
      return M.succeed((memo) =>
        M.chain_(memo.getOrElseMemoize(_I.layer), (a) => memo.getOrElseMemoize(_I.f(a)))
      );
    }
    case LayerInstructionTag.ZipWithPar: {
      return M.succeed((memo) =>
        M.zipWithPar_(memo.getOrElseMemoize(_I.layer), memo.getOrElseMemoize(_I.that), _I.f)
      );
    }
    case LayerInstructionTag.ZipWithSeq: {
      return M.succeed((memo) =>
        M.map2_(memo.getOrElseMemoize(_I.layer), memo.getOrElseMemoize(_I.that), _I.f)
      );
    }
...
export function pure<T>(has: H.Tag<T>): (resource: T) => Layer<unknown, never, H.Has<T>> {
  return (resource) =>
    new LayerManagedInstruction(
      M.chain_(M.fromEffect(I.pure(resource)), (a) => environmentFor(has, a))
    );
}
...
export function fromEffect<T>(
  has: H.Tag<T>
): <R, E>(resource: I.IO<R, E, T>) => Layer<R, E, H.Has<T>> {
  return (resource) =>
    new LayerManagedInstruction(M.chain_(M.fromEffect(resource), (a) => environmentFor(has, a)));
}
...
export class MemoMap {
  constructor(
    readonly ref: XRM.URefM<ReadonlyMap<PropertyKey, readonly [I.FIO<any, any>, Finalizer]>>
  ) {}
                            ...
                            switch (e._tag) {
                              case "Failure": {
                                return pipe(
                                  ...
                                  I.flatMap(
                                    () =>
                                      M.releaseAll(e, sequential)(innerReleaseMap) as I.FIO<E, any>
                                  ),
                              ...
                              case "Success": {
                                return pipe(
                                  ...
                                  I.bindS("outerFinalizer", () =>
                                    RelMap.add((e) => I.flatMap_(finalizerRef.get, (f) => f(e)))(
                                      outerReleaseMap
                                    )
                                  ),
              ...
              I.map(({ memoized, resource }) =>
                tuple(
                  ...
                  insert(layer.hash.get, memoized)(m) as ReadonlyMap<
                    PropertyKey,
                    readonly [I.FIO<any, any>, Finalizer]
                  >
                )
              )
            );
...
export function restrict<Tags extends H.Tag<any>[]>(
  ...
  layer: Layer<
    ...
    E,
    UnionToIntersection<
      { [k in keyof Tags]: [Tags[k]] extends [H.Tag<infer A>] ? H.Has<A> : never }[number]
    >
  >
) => Layer<
  R,
...
export function region<K, T>(
  ...
  return (_) =>
    pipe(
      fromRawEffect(I.asks((r: T): H.Has<H.Region<T, K>> => ({ [h.key]: r } as any))),
      andTo(_, "no-erase")
    );
}
