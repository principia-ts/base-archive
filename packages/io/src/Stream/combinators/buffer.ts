...
export function bufferSliding_<R, E, O>(ma: Stream<R, E, O>, capacity = 2): Stream<R, E, O> {
  return new Stream(
    M.gen(function* (_) {
      const queue = yield* _(
        I.toManaged_(Q.makeSliding<[Take.Take<E, O>, Promise<never, void>]>(capacity), (q) => q.shutdown)
      )
...
export function bufferDropping_<R, E, O>(ma: Stream<R, E, O>, capacity = 2): Stream<R, E, O> {
  return new Stream(
    M.gen(function* (_) {
      const queue = yield* _(
        I.toManaged_(Q.makeDropping<[Take.Take<E, O>, Promise<never, void>]>(capacity), (q) => q.shutdown)
      )
      return yield* _(bufferSignal_(ma, queue))
    })
  )
