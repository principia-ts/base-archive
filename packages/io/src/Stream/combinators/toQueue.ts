...
export function toQueue(capacity = 2): <R, E, O>(ma: Stream<R, E, O>) => M.Managed<R, never, XQ.Dequeue<Take<E, O>>> {
  return (ma) => toQueue_(ma, capacity)
}

export function toQueueUnbounded<R, E, O>(ma: Stream<R, E, O>): M.Managed<R, never, XQ.Dequeue<Take<E, O>>> {
