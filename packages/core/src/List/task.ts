...
export function foreachTask_<A, R, E, B>(
  l: List<A>,
  f: (a: A) => T.Task<R, E, B>
): T.Task<R, E, List<B>> {
...
export function foreachTask<A, R, E, B>(
  f: (a: A) => T.Task<R, E, B>
): (l: List<A>) => T.Task<R, E, List<B>> {
...
export function dropWhileTask<A, R, E>(
  p: (a: A) => T.Task<R, E, boolean>
): (l: List<A>) => T.Task<R, E, List<A>> {
  return (l) => dropWhileTask_(l, p);
}

export function filterTask_<A, R, E>(
  l: List<A>,
  p: (a: A) => T.Task<R, E, boolean>
): T.Task<R, E, List<A>> {
  return T.suspend(() => {
...
export function filterTask<A, R, E>(
  p: (a: A) => T.Task<R, E, boolean>
): (l: List<A>) => T.Task<R, E, List<A>> {
