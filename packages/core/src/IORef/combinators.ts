...
import type { IORef, URef } from "./model";
...
export function collect<B, C>(
  pf: (_: B) => Option<C>
): <EA, EB, A>(_: IORef<EA, EB, A, B>) => IORef<EA, Option<EB>, A, C> {
...
export function modifySome<B>(
  def: B
): <A>(f: (a: A) => Option<[B, A]>) => <EA, EB>(self: IORef<EA, EB, A, A>) => I.FIO<EA | EB, B> {
  return (f) => (self) =>
    pipe(
      self,
      concrete,
 ...
 * undefined on the current value it doesn't change it.
 */
export function getAndUpdateSome_<EA, EB, A>(self: IORef<EA, EB, A, A>, f: (a: A) => Option<A>) {
