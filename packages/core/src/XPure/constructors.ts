...
export function total<A, S1 = unknown, S2 = never>(thunk: () => A): XPure<S1, S2, unknown, never, A> {
...
export function modify<S1, S2, A>(f: (s: S1) => readonly [S2, A]): XPure<S1, S2, unknown, never, A> {
   return new ModifyInstruction(f);
}

export function suspend<S1, S2, R, E, A>(factory: () => XPure<S1, S2, R, E, A>): XPure<S1, S2, R, E, A> {
...
export function partial_<A, E>(f: () => A, onThrow: (reason: unknown) => E): XPure<unknown, never, unknown, E, A> {
   return new PartialInstruction(f, onThrow);
}

export function partial<E>(onThrow: (reason: unknown) => E): <A>(f: () => A) => XPure<unknown, never, unknown, E, A> {
