...
export function runStateEither<S1>(
  s: S1
): <S2, E, A>(fa: XPure<S1, S2, unknown, E, A>) => Either<E, readonly [S2, A]> {
 ...
 */
export function run<S1>(
  s: S1
): <S2, A>(self: XPure<S1, S2, unknown, never, A>) => readonly [S2, A] {
...
export function runEitherEnv<R>(
  env: R
): <E, A>(self: XPure<never, unknown, R, E, A>) => Either<E, A> {
