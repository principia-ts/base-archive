...
export function getApply<F extends HKT.URIS, C = HKT.Auto>(
  F: TC.Apply<F, C>
): TC.Apply<HKT.AppendURI<F, E.URI>, V<C>>;
export function getApply<F>(
  F: TC.Apply<HKT.UHKT<F>>
): TC.Apply<HKT.AppendURI<HKT.UHKT<F>, E.URI>, E.V> {
  return HKT.instance<TC.Apply<HKT.AppendURI<HKT.UHKT<F>, E.URI>, E.V>>({
    ...getApplyComposition(F, E.Apply)
  });
}

export function getMonad<F extends HKT.URIS, C = HKT.Auto>(
  M: TC.Monad<F, C>
): TC.Monad<HKT.AppendURI<F, E.URI>, V<C>>;
export function getMonad<F>(
  M: TC.Monad<HKT.UHKT<F>>
): TC.Monad<HKT.AppendURI<HKT.UHKT<F>, E.URI>, E.V> {
  const pure = pureF(M);
  return HKT.instance<TC.Monad<HKT.AppendURI<HKT.UHKT<F>, E.URI>, E.V>>({
    ...
    flatten: <E, A, D>(mma: HKT.HKT<F, Either<E, HKT.HKT<F, Either<D, A>>>>) =>
      pipe(
        mma,
        M.map(E.fold((e) => pure(E.widenE<D>()(E.left(e))), M.map(E.widenE<E>()))),
        M.flatten
      ),
    unit: () => pure(E.unit())
  });
