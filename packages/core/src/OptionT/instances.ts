...
export function getMonad<F extends HKT.URIS, C = HKT.Auto>(
  M: P.Monad<F, C>
): P.Monad<HKT.AppendURI<F, O.URI>, C>;
export function getMonad<F>(
  M: P.Monad<HKT.UHKT<F>>
): P.Monad<HKT.AppendURI<HKT.UHKT<F>, O.URI>, HKT.Auto> {
  const pure = pureF(M);
...
export function getApply<F extends HKT.URIS, C = HKT.Auto>(
  F: P.Apply<F, C>
): P.Apply<HKT.AppendURI<F, O.URI>, C>;
export function getApply<F>(
  F: P.Apply<HKT.UHKT<F>>
): P.Apply<HKT.AppendURI<HKT.UHKT<F>, O.URI>, HKT.Auto> {
