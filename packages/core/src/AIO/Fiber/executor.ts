...
import { AIOInstructionTag } from "./_internal/aio";
...
export class InterruptExit {
  ...
  constructor(readonly apply: (a: any) => T.AIO<any, any, any>) {}
}
...
/**
 * `Executor` provides all of the context and facilities required to run a `AIO`
 */
export class Executor<E, A> implements RuntimeFiber<E, A> {
  ...
  private complete<R, R1, R2, E2, A2, R3, E3, A3>(
    ...
    loser: Fiber<any, any>,
    cont: (exit: Exit<any, any>, fiber: Fiber<any, any>) => T.AIO<any, any, any>,
    winnerExit: Exit<any, any>,
    ab: AtomicReference<boolean>,
    cb: (_: T.AIO<R & R1 & R2 & R3, E2 | E3, A2 | A3>) => void
  ): void {
          ...
          while (current != null) {
            if (!this.shouldInterrupt) {
              if (opCount === this.maxOperations) {
                ...
                switch (current._tag) {
                  case AIOInstructionTag.Chain: {
                    const nested: T.Instruction = current.aio[T._I];
                    const continuation: (a: any) => T.AIO<any, any, any> = current.f;

                    switch (nested._tag) {
                      case AIOInstructionTag.Succeed: {
                        current = continuation(nested.value)[T._I];
                        break;
                      }
                      case AIOInstructionTag.Total: {
                        current = continuation(nested.thunk())[T._I];
                        break;
                      }
                      case AIOInstructionTag.Partial: {
                        try {
                          current = continuation(nested.thunk())[T._I];
                        } catch (e) {
                  ...
                  case AIOInstructionTag.Integration: {
                  ...
                  case AIOInstructionTag.Fold: {
                    this.pushContinuation(current);
                    current = current.aio[T._I];
                    break;
                  }

                  case AIOInstructionTag.SetInterrupt: {
                  ...
                  case AIOInstructionTag.GetInterrupt: {
                  ...
                  case AIOInstructionTag.Fork: {
                    current = this.next(this.fork(current.aio[T._I], current.scope));
                    break;
                  }

                  case AIOInstructionTag.CheckDescriptor: {
                    current = current.f(this.getDescriptor())[T._I];
                    break;
                  }

                  case AIOInstructionTag.Yield: {
                    current = undefined;
                    this.evaluateLater(T.unit()[T._I]);
                    break;
                  }

                  case AIOInstructionTag.Read: {
                    current = current.f(this.environments?.value || {})[T._I];
                    break;
                  }

                  case AIOInstructionTag.Give: {
                    const c = current;
                    current = T.bracket_(
                      T.total(() => {
                        this.pushEnv(c.env);
                  ...
                  case AIOInstructionTag.Suspend: {
                    current = current.factory()[T._I];
                    break;
                  }

                  case AIOInstructionTag.SuspendPartial: {
                    const c = current;
                  ...
                  case AIOInstructionTag.NewFiberRef: {
                  ...
                  case AIOInstructionTag.ModifyFiberRef: {
                    const c = current;
                  ...
                  case AIOInstructionTag.Race: {
                    current = this.raceWithImpl(current)[T._I];
                    break;
                  }

                  case AIOInstructionTag.Supervise: {
                    const c = current;
                  ...
                  case AIOInstructionTag.GetForkScope: {
                    current = current.f(
                  ...
                  case AIOInstructionTag.OverrideForkScope: {
                    const c = current;
                    current = T.bracket_(
                      T.total(() => {
