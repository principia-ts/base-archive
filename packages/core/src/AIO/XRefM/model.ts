...
export interface XRefM<RA, RB, EA, EB, A, B> {
  ...
  readonly foldM: <RC, RD, EC, ED, C, D>(
    ea: (_: EA) => EC,
    eb: (_: EB) => ED,
    ca: (_: C) => T.AIO<RC, EC, A>,
    bd: (_: B) => T.AIO<RD, ED, D>
...
export class DerivedAll<RA, RB, EA, EB, A, B, S> implements XRefM<RA, RB, EA, EB, A, B> {
  ...
  constructor(
    ...
    readonly getEither: (s: S) => T.AIO<RB, EB, B>,
    readonly setEither: (a: A) => (s: S) => T.AIO<RA, EA, S>
  ) {}
  ...
  get: T.AIO<RB, EB, B> = T.chain_(this.value.get, (a) => this.getEither(a));

  set: (a: A) => T.AIO<RA, EA, void> = (a) =>
...
export class Derived<RA, RB, EA, EB, A, B, S> implements XRefM<RA, RB, EA, EB, A, B> {
  ...
  constructor(
    readonly value: Atomic<S>,
    readonly getEither: (s: S) => T.AIO<RB, EB, B>,
    readonly setEither: (a: A) => T.AIO<RA, EA, S>
  ) {}
...
export class Atomic<A> implements XRefM<unknown, unknown, never, never, A, A> {
  ...
  readonly get: T.AIO<unknown, never, A> = this.ref.get;

  readonly set: (a: A) => T.AIO<unknown, never, void> = (a) =>
