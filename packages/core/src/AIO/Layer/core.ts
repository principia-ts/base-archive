...
import type { DefaultEnv } from "../AIO";
...
export abstract class Layer<R, E, A> {
  ...
  use<R1, E1, A1>(aio: T.AIO<R1 & A, E1, A1>): T.AIO<R & R1, E | E1, A1> {
    return M.use_(build(this["+++"](identity<R1>())), (a) => T.giveAll_(aio, a));
  }
}
...
export function pure<T>(has: H.Tag<T>): (resource: T) => Layer<unknown, never, H.Has<T>> {
  return (resource) =>
    new LayerManagedInstruction(
      M.chain_(M.fromEffect(T.pure(resource)), (a) => environmentFor(has, a))
    );
}
...
export function prepare<T>(has: H.Tag<T>) {
  return <R, E, A extends T>(acquire: T.AIO<R, E, A>) => ({
    open: <R1, E1>(open: (_: A) => T.AIO<R1, E1, any>) => ({
      release: <R2>(release: (_: A) => T.AIO<R2, never, any>) =>
        fromManaged(has)(
          M.chain_(
...
export function fromEffect<T>(
  has: H.Tag<T>
): <R, E>(resource: T.AIO<R, E, T>) => Layer<R, E, H.Has<T>> {
  return (resource) =>
...
export function fromRawEffect<R, E, A>(resource: T.AIO<R, E, A>): Layer<R, E, A> {
  return new LayerManagedInstruction(M.fromEffect(resource));
}

export function fromRawFunction<A, B>(f: (a: A) => B): Layer<A, never, B> {
  return fromRawEffect(T.asks(f));
}

export function fromRawFunctionM<A, R, E, B>(f: (a: A) => T.AIO<R, E, B>): Layer<R & A, E, B> {
  return fromRawEffect(T.asksM(f));
}
          ...
          if (inMap) {
            ...
            return pipe(
              ...
              T.map(({ memoized, resource }) =>
                tuple(
                  resource as T.AIO<readonly [R, ReleaseMap], E, readonly [Finalizer, A]>,
