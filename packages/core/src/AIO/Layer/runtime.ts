...
import type { Runtime } from "../AIO/combinators/runtime";
import { makeRuntime } from "../AIO/combinators/runtime";
...
export function region<K, T>(
  ...
  return (_) =>
    pipe(
      L.fromRawEffect(T.asks((r: T): Has<Region<T, K>> => ({ [h.key]: r } as any))),
      L.andTo(_, "no-erase")
    );
}
