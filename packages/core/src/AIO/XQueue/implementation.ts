...
export function unsafeCreate<A>(
  ...
  return new (class extends XQueue<unknown, unknown, never, never, A, A> {
    ...
    offer: (a: A) => T.AIO<unknown, never, boolean> = (a) =>
      T.suspend(() => {
        if (shutdownFlag.get) {
          return T.interrupt;
        } else {
    ...
    offerAll: (as: Iterable<A>) => T.AIO<unknown, never, boolean> = (as) => {
    ...
    take: T.AIO<unknown, never, A> = T.descriptorWith((d) =>
      T.suspend(() => {
        if (shutdownFlag.get) {
          return T.interrupt;
        }
    ...
    takeAll: T.AIO<unknown, never, readonly A[]> = T.suspend(() => {
      if (shutdownFlag.get) {
        return T.interrupt;
      } else {
        return T.total(() => {
    ...
    takeUpTo: (n: number) => T.AIO<unknown, never, readonly A[]> = (max) =>
      T.suspend(() => {
        if (shutdownFlag.get) {
          return T.interrupt;
        } else {
...
export function createQueue<A>(
  strategy: Strategy<A>
): (queue: MutableQueue<A>) => T.AIO<unknown, never, Queue<A>> {
  return (queue) =>
