...
export function takeBetween(min: number, max: number) {
  ...
  ): T.AIO<RB, EB, readonly B[]> => {
    function takeRemaining(n: number): T.AIO<RB, EB, ReadonlyArray<B>> {
      if (n <= 0) {
        return T.pure([]);
      } else {
...
export function offer<A>(
  a: A
): <RA, RB, EA, EB, B>(self: XQueue<RA, RB, EA, EB, A, B>) => T.AIO<RA, EA, boolean> {
...
export function takeAllUpTo(
  n: number
): <RA, RB, EA, EB, A, B>(self: XQueue<RA, RB, EA, EB, A, B>) => T.AIO<RB, EB, readonly B[]> {
...
export function zipWithM_<RA, RB, EA, EB, RA1, RB1, EA1, EB1, A1 extends A, C, B, R3, E3, D, A>(
  ...
  return new (class extends XQueue<RA & RA1, RB & RB1 & R3, EA | EA1, E3 | EB | EB1, A1, D> {
    ...
    offer: (a: A1) => T.AIO<RA & RA1, EA1 | EA, boolean> = (a) =>
      T.zipWithPar_(self.offer(a), that.offer(a), (x, y) => x && y);

    offerAll: (as: Iterable<A1>) => T.AIO<RA & RA1, EA1 | EA, boolean> = (as) =>
    ...
    take: T.AIO<RB & RB1 & R3, E3 | EB | EB1, D> = T.chain_(
      T.zipPar_(self.take, that.take),
      ([b, c]) => f(b, c)
    );

    takeAll: T.AIO<RB & RB1 & R3, E3 | EB | EB1, readonly D[]> = T.chain_(
    ...
    takeUpTo: (n: number) => T.AIO<RB & RB1 & R3, E3 | EB | EB1, readonly D[]> = (max) =>
...
export function bimapM_<RA, RB, EA, EB, A, B, C, RC, EC, RD, ED, D>(
  ...
  return new (class extends XQueue<RC & RA, RD & RB, EC | EA, ED | EB, C, D> {
    ...
    offer: (a: C) => T.AIO<RC & RA, EA | EC, boolean> = (c) => T.chain_(f(c), self.offer);

    offerAll: (as: Iterable<C>) => T.AIO<RC & RA, EC | EA, boolean> = (cs) =>
    ...
    take: T.AIO<RD & RB, ED | EB, D> = T.chain_(self.take, g);

    takeAll: T.AIO<RD & RB, ED | EB, readonly D[]> = T.chain_(self.takeAll, (a) =>
      T.foreach_(a, g)
    );

    takeUpTo: (n: number) => T.AIO<RD & RB, ED | EB, readonly D[]> = (max) =>
 ...
 */
export function contramapM<C, RA2, EA2, A>(f: (c: C) => T.AIO<RA2, EA2, A>) {
...
export function filterInputM_<RA, RB, EA, EB, B, A, A1 extends A, R2, E2>(
  ...
  return new (class extends XQueue<RA & R2, RB, EA | E2, EB, A1, B> {
    ...
    offer: (a: A1) => T.AIO<RA & R2, EA | E2, boolean> = (a) =>
      T.chain_(f(a), (b) => (b ? self.offer(a) : T.pure(false)));

    offerAll: (as: Iterable<A1>) => T.AIO<RA & R2, EA | E2, boolean> = (as) =>
      pipe(
        as,
        T.foreach((a) =>
          pipe(
    ...
    take: T.AIO<RB, EB, B> = self.take;

    takeAll: T.AIO<RB, EB, readonly B[]> = self.takeAll;

    takeUpTo: (n: number) => T.AIO<RB, EB, readonly B[]> = (max) => self.takeUpTo(max);
  })();
}
 ...
 */
export function mapM<B, R2, E2, C>(f: (b: B) => T.AIO<R2, E2, C>) {
