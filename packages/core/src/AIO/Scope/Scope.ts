...
export interface CommonScope<A> {
   ...
   * Returns an AIO that will succeed with `true` if the scope is closed,
   * and `false` otherwise.
   */
   ...
   * Returns an AIO that will succeed with `true` if the scope is open,
   * and `false` otherwise.
   */
...
const noCauseAIO: IO<Cause<never>> = T.pure(noCause);
...
export class LocalScope<A> implements CommonScope<A> {
  ...
  unsafeRelease(): IO<any> | null {
    if (this.references.decrementAndGet() === 0) {
      ...
      return T.uncause(
        A.reduce_(sorted, noCauseAIO, (acc, o) =>
