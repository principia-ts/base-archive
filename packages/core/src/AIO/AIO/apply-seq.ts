...
export function ap<R, E, A>(
  fa: AIO<R, E, A>
): <Q, D, B>(fab: AIO<Q, D, (a: A) => B>) => AIO<Q & R, E | D, B> {
  return (fab) => ap_(fab, fa);
}

export function apFirst_<R, E, A, Q, D, B>(
  fa: AIO<R, E, A>,
  fb: AIO<Q, D, B>
): AIO<Q & R, D | E, A> {
...
export function apFirst<Q, D, B>(
  fb: AIO<Q, D, B>
): <R, E, A>(fa: AIO<R, E, A>) => AIO<Q & R, D | E, A> {
 ...
 * @since 1.0.0
 */
export function apSecond_<R, E, A, Q, D, B>(
  fa: AIO<R, E, A>,
  fb: AIO<Q, D, B>
): AIO<Q & R, D | E, B> {
  return ap_(
...
export function apSecond<Q, D, B>(
  fb: AIO<Q, D, B>
): <R, E, A>(fa: AIO<R, E, A>) => AIO<Q & R, D | E, B> {
...
export function zipWith<A, Q, D, B, C>(
  fb: AIO<Q, D, B>,
  f: (a: A, b: B) => C
): <R, E>(fa: AIO<R, E, A>) => AIO<Q & R, D | E, C> {
