 ...
 */
export function timedWith_<R, E, A, R1, E1>(ma: AIO<R, E, A>, msTime: AIO<R1, E1, number>) {
...
export function timedWith<R1, E1>(
  msTime: AIO<R1, E1, number>
): <R, E, A>(ma: AIO<R, E, A>) => AIO<R & R1, E1 | E, [number, A]> {
 ...
 */
export function timed<R, E, A>(ma: AIO<R, E, A>) {
  return timedWith_(ma, currentTime);
}

/**
 * Returns an AIO that will timeout this effect, returning either the
...
export function timeoutTo<A, B, B1>(d: number, b: B, f: (a: A) => B1) {
  return <R, E>(ma: AIO<R, E, A>) => timeoutTo_(ma, d, b, f);
}

/**
 * Returns an AIO that will timeout this effect, returning `None` if the
 ...
 */
export function timeout_<R, E, A>(ma: AIO<R, E, A>, d: number) {
...
export function timeout(d: number) {
  return <R, E, A>(ma: AIO<R, E, A>) => timeout_(ma, d);
}
...
export function timeoutFail<E1>(d: number, e: () => E1) {
  return <R, E, A>(ma: AIO<R, E, A>) => timeoutFail_(ma, d, e);
}
