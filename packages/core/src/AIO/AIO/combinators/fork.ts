...
/**
 * Returns an AIO that forks all of the specified values, and returns a
 * composite fiber that produces a list of their results, in order.
 */
export function forkAll<R, E, A>(
  efs: Iterable<AIO<R, E, A>>
 ...
 */
export function forkAllUnit<R, E, A>(efs: Iterable<AIO<R, E, A>>): RIO<R, void> {
 ...
 */
export function forkAs_<R, E, A>(fa: AIO<R, E, A>, name: string): RIO<R, Fiber.Executor<E, A>> {
 ...
 * Forks the effect into a new independent fiber, with the specified name.
 */
export function forkAs(name: string): <R, E, A>(ef: AIO<R, E, A>) => RIO<R, Fiber.Executor<E, A>> {
