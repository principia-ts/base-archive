...
/**
 * Imports an asynchronous effect into an `AIO`. This formulation is
 * necessary when the effect is itself expressed in terms of `AIO`.
 */
export function asyncM<R, E, R1, E1, A>(
  register: (resolve: (_: AIO<R1, E1, A>) => void) => AIO<R, E, any>
): AIO<R & R1, E | E1, A> {
  return pipe(
    _.do,
