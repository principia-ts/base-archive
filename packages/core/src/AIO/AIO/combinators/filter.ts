 ...
 */
export function filter<A, R, E>(f: (a: A) => AIO<R, E, boolean>) {
...
export function filter_<A, R, E>(
  as: Iterable<A>,
  f: (a: A) => AIO<R, E, boolean>
): AIO<R, E, readonly A[]> {
  return I.reduce_(as, pure([]) as AIO<R, E, A[]>, (ma, a) =>
 ...
 */
export function filterPar_<A, R, E>(as: Iterable<A>, f: (a: A) => AIO<R, E, boolean>) {
  return pipe(
    as,
...
export function filterPar<A, R, E>(
  f: (a: A) => AIO<R, E, boolean>
): (as: Iterable<A>) => AIO<R, E, readonly A[]> {
...
export function filterParN_(
  n: number
): <A, R, E>(as: Iterable<A>, f: (a: A) => AIO<R, E, boolean>) => AIO<R, E, readonly A[]> {
  return (as, f) =>
    pipe(
      as,
...
export function filterParN(
  n: number
): <A, R, E>(f: (a: A) => AIO<R, E, boolean>) => (as: Iterable<A>) => AIO<R, E, readonly A[]> {
 ...
 */
export function filterNot_<A, R, E>(as: Iterable<A>, f: (a: A) => AIO<R, E, boolean>) {
  return filter_(
    as,
    flow(
      f,
 ...
 */
export function filterNotPar_<A, R, E>(as: Iterable<A>, f: (a: A) => AIO<R, E, boolean>) {
  return filterPar_(
    as,
    flow(
      f,
...
export function filterOrElse_<R, E, A, R1, E1, A1>(
  ...
  return chain_(fa, (a): AIO<R1, E1, A | A1> => (predicate(a) ? pure(a) : or(a)));
}
...
export function filterOrElse<A, B extends A>(
  ...
  or: (a: A) => AIO<R1, E1, A1>
) => <R, E>(fa: AIO<R, E, A>) => AIO<R & R1, E | E1, A | A1>;
...
export function filterOrFail<A, B extends A>(
  refinement: Refinement<A, B>
): <E1>(failWith: (a: A) => E1) => <R, E>(fa: AIO<R, E, A>) => AIO<R, E | E1, B>;
export function filterOrFail<A>(
  predicate: Predicate<A>
): <E1>(failWith: (a: A) => E1) => <R, E>(fa: AIO<R, E, A>) => AIO<R, E | E1, A>;
export function filterOrFail<A>(
  predicate: Predicate<A>
): <E1>(failWith: (a: A) => E1) => <R, E>(fa: AIO<R, E, A>) => AIO<R, E | E1, A> {
...
export function filterOrDie<A, B extends A>(
  refinement: Refinement<A, B>
): (dieWith: (a: A) => unknown) => <R, E>(fa: AIO<R, E, A>) => AIO<R, E, A>;
...
export function filterOrDieMessage<A, B extends A>(
  refinement: Refinement<A, B>
): (message: (a: A) => string) => <R, E>(fa: AIO<R, E, A>) => AIO<R, E, A>;
