...
export function reduceAll_<R, E, A>(as: NA.NonEmptyArray<AIO<R, E, A>>, f: (b: A, a: A) => A) {
  return A.reduce_(NA.tail(as), NA.head(as), (b, a) => zipWith_(b, a, f));
}

export function reduceAll<A>(
  f: (b: A, a: A) => A
): <R, E>(as: NA.NonEmptyArray<AIO<R, E, A>>) => AIO<R, E, A> {
  return (as) => reduceAll_(as, f);
}

export function reduceAllPar_<R, E, A>(as: NA.NonEmptyArray<AIO<R, E, A>>, f: (b: A, a: A) => A) {
  return map_(
    ...
    O.getOrElse(() => {
      throw new NoSuchElementException("AIO.reduceAllPar_");
    })
  );
}
...
export function reduceAllParN_(n: number) {
  return <R, E, A>(as: NA.NonEmptyArray<AIO<R, E, A>>, f: (b: A, a: A) => A) =>
    map_(
      ...
      O.getOrElse(() => {
        throw new NoSuchElementException("AIO.reduceAllParN_");
      })
    );
}
...
export function reduceAllParN(
  n: number
): <A>(f: (b: A, a: A) => A) => <R, E>(as: NA.NonEmptyArray<AIO<R, E, A>>) => AIO<R, E, A> {
