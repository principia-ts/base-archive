...
const mergeInterruption = <E1, A, A1>(a: A) => (x: Exit<E1, A1>): AIO<unknown, E1, A> => {
  switch (x._tag) {
    case "Success":
      return _.pure(a);
    case "Failure":
...
/**
 * Returns an AIO that races this effect with the specified effect,
...
export function race<R1, E1, A1>(
  that: AIO<R1, E1, A1>
): <R, E, A>(ef: AIO<R, E, A>) => AIO<R & R1, E1 | E, A1 | A> {
...
export function raceEither<R1, E1, A1>(
  that: AIO<R1, E1, A1>
): <R, E, A>(fa: AIO<R, E, A>) => AIO<R & R1, E1 | E, Either<A, A1>> {
 ...
 * in the background.
 */
export function raceFirst<R1, E1, A1>(that: AIO<R1, E1, A1>) {
  return <R, E, A>(ef: AIO<R, E, A>): AIO<R & R1, E | E1, A | A1> =>
    pipe(
...
/**
 * Returns an AIO that races this effect with all the specified effects,
...
export function raceAll<R, E, A>(
  ...
  return pipe(
        ...
        pipe(
          ...
          _.bindS("c", ({ fs, inheritRefs }) =>
            pipe(
              ...
              onInterrupt(() =>
                A.reduce_(fs, _.unit(), (io, f) => _.tap_(io, () => Fiber.interrupt(f)))
              )
            )
          ),
          _.map(({ c, fs }) => ({ c, fs }))
    ...
    _.tap(({ c: { fs } }) =>
      interruptStrategy === "wait" ? _.foreach_(fs, (f) => f.await) : _.unit()
    ),
    _.map(({ c: { c } }) => c)
  );
}
