...
import type { AIO, EIO } from "../model";
import { asksM } from "../reader";

/**
 * Creates a `AIO` value that represents the exit value of the specified
 * fiber.
 */
 ...
 * fiber.
 */
export function fromFiberM<R, E, A, E1>(fiber: AIO<R, E, Fiber.Fiber<E1, A>>): AIO<R, E | E1, A> {
  return chain_(fiber, Fiber.join);
}

/**
 * Lifts an `Either` into an `AIO`
 */
export function fromEither<E, A>(f: () => E.Either<E, A>): AIO<unknown, E, A> {
  return chain_(total(f), E.fold(fail, pure));
}

/**
 * Lifts an `Option` into a `AIO` but preserves the error as an option in the error channel, making it easier to compose
 * in some scenarios.
 */
...
/**
 * Create an AIO that when executed will construct `promise` and wait for its result,
...
/**
 * Creates a `AIO` from a non-throwing function
 */
...
/**
 * Creates a `AIO` from an AIO-returning function
 */
export function fromFunctionM<R, E, A>(f: (r: R) => EIO<E, A>): AIO<R, E, A> {
  return asksM(f);
}
