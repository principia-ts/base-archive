...
const _loop = <R, E, A, R1, O, R2, E2, A2>(
  fa: AIO<R, E, A>,
  orElse: (e: E, o: O) => AIO<R2, E2, A2>,
  driver: ScheduleExecutor<R1 & HasClock, E, O>
): AIO<R & R1 & R2 & HasClock, E2, Either<A2, A>> =>
  pipe(
    fa,
    map(E.right),
    catchAll((e) =>
...
/**
 * Returns an AIO that retries this effect with the specified schedule when it fails, until
...
export function retryOrElseEither_<R, E, A, R1, O, R2, E2, A2>(
  ...
  orElse: (e: E, o: O) => AIO<R2, E2, A2>
): AIO<R & R1 & R2 & HasClock, E2, E.Either<A2, A>> {
  return pipe(
    policy,
    S.driver,
...
export function retryOrElseEither<E, R1, O, R2, E2, A2>(
  ...
  return <R, A>(fa: AIO<R, E, A>) => retryOrElseEither_(fa, policy, orElse);
}
...
export function retryOrElse<E, R1, O, R2, E2, A2>(
  ...
  return <R, A>(fa: AIO<R, E, A>) => retryOrElse_(fa, policy, orElse);
}
...
export function retry<R1, E, O>(policy: S.Schedule<R1, E, O>) {
  return <R, A>(fa: AIO<R, E, A>): AIO<R & R1 & HasClock, E, A> => retry_(fa, policy);
}
 ...
 */
export function retryUntil_<R, E, A>(fa: AIO<R, E, A>, f: (e: E) => boolean): AIO<R, E, A> {
 ...
 * Retries this effect until its error satisfies the specified effectful predicate.
 */
export function retryUntil<E>(f: (e: E) => boolean): <R, A>(fa: AIO<R, E, A>) => AIO<R, E, A> {
 ...
 */
export function retryWhile_<R, E, A>(fa: AIO<R, E, A>, f: (e: E) => boolean) {
 ...
 * Retries this effect while its error satisfies the specified predicate.
 */
export function retryWhile<E>(f: (e: E) => boolean): <R, A>(fa: AIO<R, E, A>) => AIO<R, E, A> {
