...
export function asksServicesM<SS extends Record<string, Tag<any>>>(
  ...
  f: (a: { [k in keyof SS]: [SS[k]] extends [Tag<infer T>] ? T : unknown }) => T.AIO<R, E, B>
) => T.AIO<
  R &
    UnionToIntersection<
...
export function asksServiceM<T>(
  s: Tag<T>
): <R, E, B>(f: (a: T) => T.AIO<R, E, B>) => T.AIO<R & Has<T>, E, B> {
...
export function asksServiceF<T>(
  ...
  K extends keyof T &
    { [k in keyof T]: T[k] extends (...args: any[]) => T.AIO<any, any, any> ? k : never }[keyof T]
>(
  k: K
) => (
  ...args: T[K] extends (...args: infer ARGS) => T.AIO<any, any, any> ? ARGS : unknown[]
) => T[K] extends (...args: any[]) => T.AIO<infer R, infer E, infer A>
  ? T.AIO<R & Has<T>, E, A>
  : unknown[] {
 ...
 * Access a service with the required Service Entry
 */
export function asksService<T>(s: Tag<T>): <B>(f: (a: T) => B) => T.AIO<Has<T>, never, B> {
 ...
 * Access a service with the required Service Entry
 */
export function askService<T>(s: Tag<T>): T.AIO<Has<T>, never, T> {
...
export function giveServiceM<T>(_: Tag<T>) {
  return <R, E>(f: T.AIO<R, E, T>) => <R1, E1, A1>(
    ma: T.AIO<R1 & Has<T>, E1, A1>
  ): T.AIO<R & R1, E | E1, A1> =>
...
export function giveService<T>(
  _: Tag<T>
): (f: T) => <R1, E1, A1>(ma: T.AIO<R1 & Has<T>, E1, A1>) => T.AIO<R1, E1, A1> {
...
export function replaceServiceM<R, E, T>(
  _: Tag<T>,
  f: (_: T) => T.AIO<R, E, T>
): <R1, E1, A1>(ma: T.AIO<R1 & Has<T>, E1, A1>) => T.AIO<R & R1 & Has<T>, E1 | E, A1> {
...
export function replaceService<T>(
  _: Tag<T>,
  f: (_: T) => T
): <R1, E1, A1>(ma: T.AIO<R1 & Has<T>, E1, A1>) => T.AIO<R1 & Has<T>, E1, A1> {
...
export function askServiceIn<A>(
  _: Tag<A>
): <K, T>(h: Tag<Region<Has<A> & T, K>>) => T.AIO<Has<Region<Has<A> & T, K>>, never, A> {
  return (h) =>
    useRegion(h)(
      asksServiceM(_)((a) =>
        pipe(
...
) => <B>(f: (_: A) => B) => T.AIO<Has<Region<Has<A> & T, K>>, never, B> {
  return (h) => (f) =>
    useRegion(h)(
      asksServiceM(_)((a) =>
        pipe(
...
export function asksServiceInM<A>(
  ...
  h: Tag<Region<Has<A> & T, K>>
) => <R, E, B>(f: (_: A) => T.AIO<R, E, B>) => T.AIO<R & Has<Region<Has<A> & T, K>>, E, B> {
  return (h) => (f) =>
    useRegion(h)(
      asksServiceM(_)((a) =>
        pipe(
 ...
 */
export function asService<A>(has: Tag<A>): <R, E>(fa: T.AIO<R, E, A>) => T.AIO<R, E, Has<A>> {
