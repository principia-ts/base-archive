...
export function asksM<R0, R, E, A>(f: (r: R0) => T.AIO<R, E, A>): Managed<R0 & R, E, A> {
...
export function gives_<R, E, A, R0>(ma: Managed<R, E, A>, f: (r0: R0) => R): Managed<R0, E, A> {
  return new Managed(
    T.asksM(([r0, rm]: readonly [R0, ReleaseMap]) => T.giveAll_(ma.aio, [f(r0), rm]))
  );
}
