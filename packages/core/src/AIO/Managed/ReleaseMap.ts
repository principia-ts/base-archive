...
export type Finalizer = (exit: Exit<any, any>) => T.AIO<unknown, never, any>;
...
export function addIfOpen(finalizer: Finalizer) {
  return (_: ReleaseMap): T.AIO<unknown, never, Option<number>> =>
    pipe(
      _.ref,
      XR.modify<T.AIO<unknown, never, Option<number>>, State>((s) => {
        switch (s._tag) {
          case "Exited": {
            return [
...
export function replace(
  key: number,
  finalizer: Finalizer
): (_: ReleaseMap) => T.AIO<unknown, never, Option<Finalizer>> {
  return (_) =>
    pipe(
      _.ref,
      XR.modify<T.AIO<unknown, never, Option<Finalizer>>, State>((s) => {
        switch (s._tag) {
          case "Exited":
