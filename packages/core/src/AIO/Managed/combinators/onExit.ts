...
export function onExit_<R, E, A, R1>(
  self: Managed<R, E, A>,
  cleanup: (exit: Exit<E, A>) => T.AIO<R1, never, any>
) {
  return new Managed<R & R1, E, A>(
    T.uninterruptibleMask(({ restore }) =>
      pipe(
        ...
        T.bindS("exitEA", (s) =>
          restore(T.giveAll_(T.result(T.map_(self.aio, ([_, a]) => a)), [s.r, s.innerReleaseMap]))
        ),
        T.bindS("releaseMapEntry", (s) =>
          add((e) =>
            pipe(
              ...
              T.result,
              T.zipWith(pipe(cleanup(s.exitEA), T.giveAll(s.r), T.result), (l, r) =>
                Ex.apSecond_(l, r)
              )
            )
          )(s.outerReleaseMap)
