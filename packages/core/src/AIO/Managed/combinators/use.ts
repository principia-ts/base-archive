...
export function use<A, R2, E2, B>(
  f: (a: A) => T.AIO<R2, E2, B>
): <R, E>(self: Managed<R, E, A>) => T.AIO<R & R2, E | E2, B> {
 ...
 */
export const useNow: <R, E, A>(ma: Managed<R, E, A>) => T.AIO<R, E, A> = use(T.pure);
 ...
 */
export const useForever: <R, E, A>(ma: Managed<R, E, A>) => T.AIO<R, E, never> = use(() => T.never);
