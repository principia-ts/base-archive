...
export function ifM_<R, E, R1, E1, B, R2, E2, C>(
  ...
  return chain_(mb, (b) =>
    b ? (onTrue() as Managed<R & R1 & R2, E | E1 | E2, B | C>) : onFalse()
  );
}
 ...
 * Runs `onTrue` if the result of `b` is `true` and `onFalse` otherwise.
 */
export function ifM<R1, E1, B, R2, E2, C>(
  onTrue: () => Managed<R1, E1, B>,
  onFalse: () => Managed<R2, E2, C>
) {
  return <R, E>(mb: Managed<R, E, boolean>): Managed<R & R1 & R2, E | E1 | E2, B | C> =>
    ifM_(mb, onTrue, onFalse);
}
