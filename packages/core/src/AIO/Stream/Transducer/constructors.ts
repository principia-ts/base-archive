...
export function fromPush<R, E, I, O>(
  push: (input: O.Option<ReadonlyArray<I>>) => T.AIO<R, E, ReadonlyArray<O>>
): Transducer<R, E, I, O> {
 ...
 */
export function fromEffect<R, E, A>(aio: T.AIO<R, E, A>): Transducer<R, E, unknown, A> {
  return new Transducer(M.succeed((_: any) => T.map_(aio, A.pure)));
}
 ...
 */
export function fromFunctionM<R, E, I, O>(f: (i: I) => T.AIO<R, E, O>): Transducer<R, E, I, O> {
...
export function branchAfter<R, E, I, O>(
  ...
  interface Emitting {
    _tag: "Emitting";
    finalizer: Finalizer;
    push: (is: O.Option<ReadonlyArray<I>>) => T.AIO<R, E, ReadonlyArray<O>>;
  }
 ...
 */
export function dropWhileM<R, E, I>(p: (i: I) => T.AIO<R, E, boolean>): Transducer<R, E, I, I> {
  return new Transducer(
    pipe(
      M.do,
        ...
        O.fold_(
          ...
          (is) =>
            pipe(
              ...
              T.chain((b) =>
                b
                  ? T.map_(A.dropWhileEffect_(is, p), (l) => [l, A.isEmpty(l)] as const)
...
export function foldM<R, E, I, O>(
  initial: O,
  cont: (o: O) => boolean,
  f: (output: O, input: I) => T.AIO<R, E, O>
): Transducer<R, E, I, O> {
  ...
  const go = (
    ...
    state: O,
    progress: boolean
  ): T.AIO<R, E, readonly [ReadonlyArray<O>, O, boolean]> =>
    A.reduce_(
      in_,
      T.succeed([A.empty(), state, progress]) as T.AIO<
        R,
        E,
        readonly [ReadonlyArray<O>, O, boolean]
      >,
 ...
 * to return a `AIO`, and consequently it allows the transducer
 * to fail.
 */
...
export function foldWeightedDecomposeM<R, E, I, O>(
  initial: O,
  costFn: (output: O, input: I) => T.AIO<R, E, number>,
  max: number,
  decompose: (input: I) => T.AIO<R, E, ReadonlyArray<I>>,
  f: (output: O, input: I) => T.AIO<R, E, O>
): Transducer<R, E, I, O> {
  interface FoldWeightedState {
    result: O;
  ...
  const go = (
    ...
    state: FoldWeightedState,
    dirty: boolean
  ): T.AIO<R, E, readonly [ReadonlyArray<O>, FoldWeightedState, boolean]> =>
    A.reduce_(
      in_,
      T.succeed([os, state, dirty]) as T.AIO<
        R,
        E,
        readonly [ReadonlyArray<O>, FoldWeightedState, boolean]
      >,
