...
export function runManaged_<R, E, A, R1, E1, B>(
  ...
  return pipe(
    ...
    M.mapM(([pull, push]) => {
      const go: T.AIO<R1 & R, E1 | E, B> = T.foldCauseM_(
        pull,
        (c): T.AIO<R1, E1 | E, B> =>
          pipe(
...
export function run<A, R1, E1, B>(
  sink: Sink.Sink<R1, E1, A, any, B>
): <R, E>(stream: Stream<R, E, A>) => T.AIO<R & R1, E1 | E, B> {
  return (stream) => run_(stream, sink);
}

export function runCollect<R, E, A>(stream: Stream<R, E, A>): T.AIO<R, E, ReadonlyArray<A>> {
 ...
 */
export function runDrain<R, E, A>(stream: Stream<R, E, A>): T.AIO<R, E, void> {
  return pipe(
    stream,
    foreach((_) => T.unit())
  );
...
export function foreach<A, R1, E1, B>(
  f: (a: A) => T.AIO<R1, E1, B>
): <R, E>(stream: Stream<R, E, A>) => T.AIO<R & R1, E1 | E, void> {
