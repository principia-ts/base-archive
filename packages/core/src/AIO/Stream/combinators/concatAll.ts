...
function go<R, E, A>(
  ...
  currStream: Ref<T.AIO<R, Option<E>, ReadonlyArray<A>>>,
  switchStream: (
    x: M.Managed<R, never, T.AIO<R, Option<E>, ReadonlyArray<A>>>
  ) => T.AIO<R, never, T.AIO<R, Option<E>, ReadonlyArray<A>>>
): T.AIO<R, Option<E>, ReadonlyArray<A>> {
  return pipe(
    currStream.get,
    T.flatten,
    T.catchAllCause((x) =>
...
export function concatAll<R, E, A>(streams: ReadonlyArray<Stream<R, E, A>>): Stream<R, E, A> {
  ...
  return new Stream(
    pipe(
      ...
      M.bindS("currStream", () => XR.makeManaged<T.AIO<R, Option<E>, ReadonlyArray<A>>>(Pull.end)),
      M.bindS("switchStream", () =>
        M.switchable<R, never, T.AIO<R, Option<E>, ReadonlyArray<A>>>()
      ),
