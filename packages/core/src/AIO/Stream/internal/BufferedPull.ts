...
export class BufferedPull<R, E, A> {
  constructor(
    readonly upstream: T.AIO<R, O.Option<E>, ReadonlyArray<A>>,
...
export function ifNotDone<R1, E1, A1>(
  fa: T.AIO<R1, O.Option<E1>, A1>
): <R, E, A>(self: BufferedPull<R, E, A>) => T.AIO<R1, O.Option<E1>, A1> {
  return (self) =>
    pipe(
      self.done.get,
...
export function update<R, E, A>(self: BufferedPull<R, E, A>): T.AIO<R, O.Option<E>, void> {
  return pipe(
    self,
    ifNotDone(
      pipe(
...
export function pullElement<R, E, A>(self: BufferedPull<R, E, A>): T.AIO<R, O.Option<E>, A> {
  return pipe(
    self,
    ifNotDone(
      pipe(
        self.cursor,
        R.modify(([c, i]): [T.AIO<R, O.Option<E>, A>, [ReadonlyArray<A>, number]] => {
          if (i >= c.length) {
            return [
              pipe(
                update(self),
...
): T.AIO<R, O.Option<E>, ReadonlyArray<A>> {
  return pipe(
    self,
    ifNotDone(
      pipe(
...
export function make<R, E, A>(
  pull: T.AIO<R, O.Option<E>, ReadonlyArray<A>>
): T.AIO<unknown, never, BufferedPull<R, E, A>> {
  return pipe(
    T.do,
