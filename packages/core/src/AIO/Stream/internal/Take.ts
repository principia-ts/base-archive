...
export function fromPull<R, E, O>(pull: Pull<R, E, O>): T.AIO<R, never, Take<E, O>> {
  return pipe(
    pull,
    T.foldCause(
      (c) =>
...
export function tap_<E, A, R, E1>(
  take: Take<E, A>,
  f: (as: ReadonlyArray<A>) => T.AIO<R, E1, any>
): T.AIO<R, E1, void> {
  return T.asUnit(Ex.foreachAIO_(take, f));
}

export function tap<A, R, E1>(
  f: (as: ReadonlyArray<A>) => T.AIO<R, E1, any>
): <E>(take: Exit<Option<E>, ReadonlyArray<A>>) => T.AIO<R, E1, void> {
...
export function foldM_<E, A, R, E1, Z>(
  ...
  error: (cause: C.Cause<E>) => T.AIO<R, E1, Z>,
  value: (chunk: ReadonlyArray<A>) => T.AIO<R, E1, Z>
): T.AIO<R, E1, Z> {
