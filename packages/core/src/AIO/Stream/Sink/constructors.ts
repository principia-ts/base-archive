 ...
 */
export function fromEffect<R, E, I, Z>(aio: T.AIO<R, E, Z>): Sink<R, E, I, I, Z> {
 ...
 */
export function fromForeach<I, R1, E1>(f: (i: I) => T.AIO<R1, E1, any>): Sink<R1, E1, I, I, void> {
  const go = (
    chunk: ReadonlyArray<I>,
    idx: number,
    len: number
  ): T.AIO<R1, [E.Either<E1, never>, ReadonlyArray<I>], void> => {
    if (idx === len) {
      return Push.more;
    } else {
      return pipe(
...
export function fromForeachChunk<R, E, I>(
  f: (chunk: ReadonlyArray<I>) => T.AIO<R, E, any>
...
export function fromFoldChunksM_<R, E, I, Z>(
  z: Z,
  cont: (z: Z) => boolean,
  f: (z: Z, i: ReadonlyArray<I>) => T.AIO<R, E, Z>
): Sink<R, E, I, I, Z> {
  return cont(z)
    ? new Sink(
...
export function fromFoldM<R, E, I, Z>(
  ...
  const foldChunk = (
    ...
    i: number,
    len: number
  ): T.AIO<R, readonly [E, ReadonlyArray<I>], readonly [Z, O.Option<ReadonlyArray<I>>]> => {
    if (i === len) {
