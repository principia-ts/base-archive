 ...
 *
 * One way to think of `Stream` is as a `AIO` program that could emit multiple values.
 *
 * This data type can emit multiple `A` values through multiple calls to `next`.
 * Similarly, embedded inside every `Stream` is an AIO program: `AIO<R, Option<E>, ReadonlyArray<A>>`.
 ...
 * facilities for its `E` type parameter, modeled similarly to `AIO` (with some
...
export class Stream<R, E, A> {
  ...
  constructor(readonly proc: M.Managed<R, never, T.AIO<R, Option<E>, ReadonlyArray<A>>>) {}
}
...
export class Chain<R_, E_, O, O2> {
  constructor(
    ...
    readonly outerStream: T.AIO<R_, Option<E_>, ReadonlyArray<O>>,
    readonly currOuterChunk: XR.Ref<[ReadonlyArray<O>, number]>,
    readonly currInnerStream: XR.Ref<T.AIO<R_, Option<E_>, ReadonlyArray<O2>>>,
  ...
  pullNonEmpty<R, E, O>(
    pull: T.AIO<R, Option<E>, ReadonlyArray<O>>
  ): T.AIO<R, Option<E>, ReadonlyArray<O>> {
    return pipe(
      pull,
  ...
  pullOuter() {
    return pipe(
      this.currOuterChunk,
      XR.modify(([chunk, nextIdx]): [T.AIO<R_, Option<E_>, O>, [ReadonlyArray<O>, number]] => {
  ...
  apply(): T.AIO<R_, Option<E_>, ReadonlyArray<O2>> {
    return pipe(
