...
import type { DefaultEnv } from "../IO";
import type { Cause } from "../IO/Cause";
import { sequential } from "../IO/ExecutionStrategy";
import type { Exit } from "../IO/Exit/model";
...
export function _build<R, E, A>(
  ...
  switch (_I._tag) {
    ...
    case LayerInstructionTag.ZipWithSeq: {
      return M.succeed((memo) =>
        M.zipWith_(memo.getOrElseMemoize(_I.layer), memo.getOrElseMemoize(_I.that), _I.f)
      );
    }
                              ...
                              case "Success": {
                                return pipe(
                                  ...
                                  I.bindS("outerFinalizer", () =>
                                    RelMap.add((e) => I.chain_(finalizerRef.get, (f) => f(e)))(
                                      outerReleaseMap
                                    )
                                  ),
              ...
              I.letS(
                ...
                ({ finalizerRef, observers, promise }) =>
                  [
                    ...
                    (e: Exit<any, any>) => I.chain_(finalizerRef.get, (f) => f(e))
...
export function makeMemoMap() {
  return pipe(
    ...
    I.chain((r) => I.total(() => new MemoMap(r)))
  );
}
