    ...
    return new Stream(
      M.withChildren((getChildren) =>
        pipe(
          ...
          M.bindS("out", () =>
            T.toManaged_(
              Q.makeBounded<T.IO<R1, O.Option<E | E1>, A.Chunk<O2>>>(bufferSize),
              (q) => q.shutdown
            )
          ),
                ...
                (_) =>
                  pipe(
                    ...
                    T.raceWith(
                      ...
                      (_, failureAwait) =>
                        pipe(out.offer(Pull.end), T.andThen(T.asUnit(F.interrupt(failureAwait))))
                    ),
                    T.toManaged()
                  )
              ),
...
export function chainParSwitch(n: number, bufferSize = 16) {
  return <O, R1, E1, O2>(f: (o: O) => Stream<R1, E1, O2>) => <R, E>(
    ma: Stream<R, E, O>
  ): Stream<R & R1, E | E1, O2> => chainParSwitch_(n, bufferSize)(ma, f);
}
