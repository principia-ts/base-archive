...
export function withEarlyReleaseExit_<R, E, A>(
   ...
   return new Managed(
      T.map_(ma.task, ([finalizer, a]) => [finalizer, [T.makeUninterruptible(finalizer(exit)), a]] as const)
   );
}
...
export function withEarlyRelease<R, E, A>(ma: Managed<R, E, A>): Managed<R, E, readonly [T.IO<unknown>, A]> {
