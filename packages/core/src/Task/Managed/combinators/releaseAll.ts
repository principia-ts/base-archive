...
export function releaseAll(exit: Exit<any, any>, execStrategy: ExecutionStrategy): (_: RM.ReleaseMap) => T.IO<any> {
   return (_) =>
      pipe(
         _.ref,
               ...
               case "Running": {
                  switch (execStrategy._tag) {
                     ...
                     case "ParallelN": {
                        return [
                           T.chain_(
                              foreachParNTask_(execStrategy.n)(Array.from(RM.finalizers(s)).reverse(), ([_, f]) =>
                                 T.result(f(exit))
                              ),
                              (e) => T.done(O.getOrElse_(Ex.collectAllPar(...e), () => Ex.succeed([])))
                           ),
