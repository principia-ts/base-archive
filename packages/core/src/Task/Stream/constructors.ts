...
const unwrap = <R, E, O>(fa: T.Task<R, E, Stream<R, E, O>>): Stream<R, E, O> =>
  flatten(fromTask(fa));
/**
 ...
 */
export function repeatTaskChunkOption<R, E, A>(
  ef: T.Task<R, Option<E>, ReadonlyArray<A>>
): Stream<R, E, A> {
  return new Stream(
    pipe(
      M.do,
...
const ensuringFirst_ = <R, E, A, R1>(
  stream: Stream<R, E, A>,
  finalizer: T.Task<R1, never, unknown>
 ...
 */
export function fromArrayXQueue<R, E, O>(
...
export function apply<R, E, A>(
  proc: M.Managed<R, never, T.Task<R, Option<E>, ReadonlyArray<A>>>
): Stream<R, E, A> {
