...
export function mapConcat_<R, E, A, B>(stream: Stream<R, E, A>, f: (a: A) => Iterable<B>) {
   return mapChunks_(stream, (chunks) => A.chain_(chunks, (a) => Array.from(f(a))));
}
...
export function mapConcatChunk_<R, E, A, B>(stream: Stream<R, E, A>, f: (a: A) => ReadonlyArray<B>): Stream<R, E, B> {
