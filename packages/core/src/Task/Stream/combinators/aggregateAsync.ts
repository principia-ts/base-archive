...
export function aggregateAsyncWithinEither_<R, E, O, R1, E1, P, Q>(
   ...
   return pipe(
      ...
      M.bindS("waitingFiber", () => XR.makeManagedRef<O.Option<Fiber<never, Take.Take<E | E1, O>>>>(O.none())),
      ...
      M.letS("consumer", ({ pull, push, handoff, raceNextTime, waitingFiber, sdriver, lastChunk, producer }) => {
                              ...
                              O.fold(
                                 () =>
                                    pipe(
                                       ...
                                       T.map(({ take, scheduleResult }) => [scheduleResult, Take.map_(take, E.right)])
                                    ),
                                 (_) =>
                                    pipe(
                                       T.do,
