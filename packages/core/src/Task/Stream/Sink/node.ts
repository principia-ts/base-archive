...
export function fromWritable<I extends string | Buffer | Uint8Array>(
  ...
  return new Sink(
    M.gen(function* (_) {
       ...
       * const wr = yield* _(
       *   M.make_(
       *     T.async<unknown, never, Writable>((resolve) => {
       *       // catch initial errors from the writable (eg. bad file name)
       *       writable.on("error", (err) => {
       *         resolve(T.die(err));
       *       });
       *       setImmediate(() => {
       *         writable.removeAllListeners("error");
       *         resolve(T.succeed(writable));
       *       });
       *     }),
       *     (w) =>
       *       T.total(() => {
       ...
       *     T.async<unknown, readonly [E.Either<Error, void>, ReadonlyArray<I>], void>(
       *       (resolve) => {
       *         resolve(Push.emit(undefined, A.empty()));
       *       }
       *     ),
       *   (in_) =>
       *     T.async<unknown, readonly [E.Either<Error, void>, ReadonlyArray<I>], void>(
       ...
       *             }
       *           }
       *           wr.removeListener("error", listen);
       *           resolve(Push.more);
       *         } catch (err) {
       *           resolve(Push.fail(err, A.empty()));
       *         }
