...
export type TupleR<T extends NonEmptyArray<Task<any, any, any>>> = UnionToIntersection<
  {
    [K in keyof T]: [T[K]] extends [Task<infer R, any, any>]
      ? unknown extends R
        ? never
        : R
      : never;
...
export function tuple<A extends NonEmptyArray<Task<any, any, any>>>(
  ...t: A
): Task<TupleR<A>, TupleE<A>, TupleA<A>> {
  return foreach_(t, identity) as any;
}

export function tuplePar<A extends NonEmptyArray<Task<any, any, any>>>(
  ...t: A
): Task<TupleR<A>, TupleE<A>, TupleA<A>> {
...
export function tupleParN(n: number) {
  return <A extends NonEmptyArray<Task<any, any, any>>>(
    ...t: A
