...
export function asksServices<SS extends Record<string, Tag<any>>>(
   ...
   f: (a: { [k in keyof SS]: [SS[k]] extends [Tag<infer T>] ? T : unknown }) => B
) => T.RIO<UnionToIntersection<{ [k in keyof SS]: [SS[k]] extends [Tag<infer T>] ? Has<T> : unknown }[keyof SS]>, B> {
   return (f) =>
      T.asks(
         (
            r: UnionToIntersection<
 ...
 */
export function asksServiceM<T>(s: Tag<T>): <R, E, B>(f: (a: T) => T.Task<R, E, B>) => T.Task<R & Has<T>, E, B> {
...
export function asksServiceF<T>(
   ...
   K extends keyof T & { [k in keyof T]: T[k] extends (...args: any[]) => T.Task<any, any, any> ? k : never }[keyof T]
>(
   k: K
) => (
...
export function giveServiceM<T>(_: Tag<T>) {
   return <R, E>(f: T.Task<R, E, T>) => <R1, E1, A1>(ma: T.Task<R1 & Has<T>, E1, A1>): T.Task<R & R1, E | E1, A1> =>
...
export function asksRegion<K, T>(h: Tag<Region<T, K>>): <A>(e: (_: T) => A) => T.Task<Has<Region<T, K>>, never, A> {
...
export function asksServiceIn<A>(
   _: Tag<A>
): <K, T>(h: Tag<Region<Has<A> & T, K>>) => <B>(f: (_: A) => B) => T.Task<Has<Region<Has<A> & T, K>>, never, B> {
   return (h) => (f) =>
      useRegion(h)(
         asksServiceM(_)((a) =>
            pipe(
