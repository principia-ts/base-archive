...
export function mapBoth_<EA, A, EB, B, C>(fa: Exit<EA, A>, fb: Exit<EB, B>, f: (a: A, b: B) => C): Exit<EA | EB, C> {
   return mapBothCause_(fa, fb, f, C.then);
}

export function mapBoth<A, G, B, C>(fb: Exit<G, B>, f: (a: A, b: B) => C): <E>(fa: Exit<E, A>) => Exit<G | E, C> {
