...
export function delayedM_<R, I, O, R1>(
  sc: Schedule<R, I, O>,
  f: (d: number) => I.IO<R1, never, number>
) {
...
export function duration(n: number) {
  return makeSchedule((now, _: unknown) =>
    I.pure(makeContinue(0, now + n, () => I.pure(makeDone(n))))
  );
}
...
const combineWithLoop = <R, I, O, R1, I1, O1>(
  ...
  return I.map_(I.zip_(left, right), ([l, r]) => {
    switch (l._tag) {
      case "Done": {
        switch (r._tag) {
          case "Done": {
...
const zipLoop = <R, I, O, R1, I1, O1>(
  ...
  I.map_(I.zip_(sc(now, in1), that(now, in2)), ([d1, d2]) => {
    switch (d1._tag) {
      case "Done": {
        switch (d2._tag) {
          case "Done": {
...
const unfoldMLoop = <R, A>(a: A, f: (a: A) => I.IO<R, never, A>): StepFunction<R, unknown, A> => (
  now,
  _
  ...
  I.chain_(step(now, i), (d) => {
    switch (d._tag) {
      ...
      case "Continue": {
        return f(d.out)
          ? sc.step(now, i)
          : I.pure(makeContinue(d.out, d.interval, resetWhenLoop(sc, d.next, f)));
      }
    }
  });
...
export function run<I>(
  now: number,
  i: Iterable<I>
): <R, O>(sc: Schedule<R, I, O>) => I.IO<R, never, readonly O[]> {
