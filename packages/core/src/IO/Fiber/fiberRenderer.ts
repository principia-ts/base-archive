...
import { zipPar_ } from "../applicative-par";
import type { UIO } from "./_internal/io";
import * as T from "./_internal/io";
import { FiberDump } from "./dump";
import { fiberName } from "./fiberName";
import type { RuntimeFiber } from "./model";
import type { FiberStatus } from "./status";

export function dump<E, A>(fiber: RuntimeFiber<E, A>): T.UIO<FiberDump> {
  return T.map_(zipPar_(fiber.getRef(fiberName), fiber.status), ([name, status]) =>
    FiberDump(fiber.id, name, status)
  );
}

export function dumpFibers(
  fibers: Iterable<RuntimeFiber<any, any>>
): UIO<ReadonlyArray<FiberDump>> {
