...
export const map = <A, B>(f: (a: A, k: number) => B) => (i: Iterable<A>): Iterable<B> => ({
...
export const chain = <A, B>(f: (a: A) => Iterable<B>) => (i: Iterable<A>): Iterable<B> => ({
   [Symbol.iterator]: () => genChain(i[Symbol.iterator](), f)
});

export const chain_ = <A, B>(i: Iterable<A>, f: (a: A) => Iterable<B>): Iterable<B> => ({
   [Symbol.iterator]: () => genChain(i[Symbol.iterator](), f)
});
...
export const reduce_ = <A, B>(fa: Iterable<A>, b: B, f: (b: B, a: A, i: number) => B): B => {
   let res = b;
   let n = -1;
   const iterator = fa[Symbol.iterator]();
   // eslint-disable-next-line no-constant-condition
