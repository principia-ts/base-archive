...
export const left = <F extends HKT.URIS, C = HKT.Auto>(M: TC.Monad<F, C>) => <E = never, A = never>(
   l: E
) => M.pure(E.left(l));

export const right = <F extends HKT.URIS, C = HKT.Auto>(M: TC.Monad<F, C>) => <
   E = never,
   A = never
>(
   r: A
...
): HKT.Kind<F, C, N, K, Q, W, X, I, S, R, E, E.Either<A, never>> =>
   M._map(ma, (a) => E.left<A, never>(a));
