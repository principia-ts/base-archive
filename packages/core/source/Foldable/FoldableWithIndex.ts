...
export function getFoldableWithIndexComposition<F, G>(
   ...
   const _reduceWithIndex: UC_ReduceWithIndexFComposition<HKT.UHKT<F>, HKT.UHKT<G>> = <KF, KG, A, B>(
   ...
   const _foldMapWithIndex: UC_FoldMapWithIndexFComposition<HKT.UHKT<F>, HKT.UHKT<G>> = <M>(M: Monoid<M>) => <
      KF,
      KG,
      A
   >(
      fga: HKT.HKT<F, HKT.HKT<G, A>>,
      f: (k: [KF, KG], a: A) => M
   ) => F._foldMapWithIndex(M)(fga, (kf: KF, ga) => G._foldMapWithIndex(M)(ga, (kg: KG, a) => f([kf, kg], a)));

   const _reduceRightWithIndex: UC_ReduceRightWithIndexFComposition<HKT.UHKT<F>, HKT.UHKT<G>> = <KF, KG, A, B>(
      fga: HKT.HKT<F, HKT.HKT<G, A>>,
      b: B,
      f: (k: [KF, KG], a: A, b: B) => B
   ) =>
