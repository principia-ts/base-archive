...
export interface WiltWithIndexF<F extends HKT.URIS, C = HKT.Auto> {
...
export interface UC_WiltWithIndexF<F extends HKT.URIS, C = HKT.Auto> {
   <G extends HKT.URIS, GC = HKT.Auto>(F: Applicative<G, GC>): <
      NF extends string,
      KF,
      QF,
...
export function implementUCWiltWithIndex<F extends HKT.URIS, C = HKT.Auto>(): (
   i: <NF extends string, KF, QF, WF, XF, IF, SF, RF, EF, A, B, B2, G>(_: {
      A: A;
      B: B;
      B2: B2;
...
export function implementUCWiltWithIndex() {
   return (i: any) => i();
}
