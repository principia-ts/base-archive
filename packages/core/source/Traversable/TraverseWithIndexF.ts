...
export interface TraverseWithIndexF<F extends HKT.URIS, CF = HKT.Auto> {
...
export interface UC_TraverseWithIndexF<F extends HKT.URIS, CF = HKT.Auto> {
   <G extends HKT.URIS, CG = HKT.Auto>(A: Applicative<G, CG>): <
      NG extends string,
      KG,
      QG,
...
export interface TraverseWithIndexFComposition<F extends HKT.URIS, G extends HKT.URIS, CF = HKT.Auto, CG = HKT.Auto> {
...
export interface UC_TraverseWithIndexFComposition<
   F extends HKT.URIS,
   G extends HKT.URIS,
   CF = HKT.Auto,
   CG = HKT.Auto
...
) => TraverseWithIndexF<F, C>;
export function implementTraverseWithIndex() {
   return (i: any) => i();
}

export function implementUCTraverseWithIndex<F extends HKT.URIS, C = HKT.Auto>(): (
   i: <N extends string, K, Q, W, X, I, S, R, E, A, B, G>(_: {
      A: A;
      B: B;
      G: G;
...
) => UC_TraverseWithIndexF<F, C>;
export function implementUCTraverseWithIndex() {
   return (i: any) => i();
}
