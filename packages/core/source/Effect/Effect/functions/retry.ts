...
import { catchAll, catchAll_, chain, chain_, fail, foldM, map, map_, pure } from "../core";
...
const _loop = <R, E, A, R1, O, R2, E2, A2>(
   ...
   driver: ScheduleDriver<R1 & HasClock, E, O>
...
export const retryOrElseEither = <E, R1, O, R2, E2, A2>(
   ...
   orElse: (e: E, o: O) => Effect<R2, E2, A2>
) => <R, A>(fa: Effect<R, E, A>) => retryOrElseEither_(fa, policy, orElse);
...
export const retryOrElse_ = <R, E, A, R1, O, R2, E2, A2>(
   ...
   map_(retryOrElseEither_(fa, policy, orElse), E.fold(identity, identity));
...
export const retryOrElse = <E, R1, O, R2, E2, A2>(
   policy: S.Schedule<R1, E, O>,
   orElse: (e: E, o: O) => Effect<R2, E2, A2>
) => <R, A>(fa: Effect<R, E, A>) => retryOrElse_(fa, policy, orElse);
...
export const retry_ = <R, E, A, R1, O>(
   fa: Effect<R, E, A>,
   policy: S.Schedule<R1, E, O>
): Effect<R & R1 & HasClock, E, A> => retryOrElse_(fa, policy, (e, _) => fail(e));
...
export const retry = <R1, E, O>(policy: S.Schedule<R1, E, O>) => <R, A>(
   fa: Effect<R, E, A>
): Effect<R & R1 & HasClock, E, A> => retry_(fa, policy);
...
): Effect<R & R1, E | E1, A> => catchAll_(fa, (e) => chain_(f(e), (b) => (b ? fail(e) : retryUntilM_(fa, f))));
...
export const retryUntilM = <E, R1, E1>(f: (e: E) => Effect<R1, E1, boolean>) => <R, A>(
   fa: Effect<R, E, A>
): Effect<R & R1, E | E1, A> => retryUntilM_(fa, f);

/**
 * Retries this effect until its error satisfies the specified predicate.
 */
export const retryUntil_ = <R, E, A>(fa: Effect<R, E, A>, f: (e: E) => boolean) => retryUntilM_(fa, flow(f, pure));

/**
 * Retries this effect until its error satisfies the specified effectful predicate.
 */
export const retryUntil = <E>(f: (e: E) => boolean) => <R, A>(fa: Effect<R, E, A>) => retryUntil_(fa, f);
...
export const retryWhileM_ = <R, E, A, R1, E1>(
   fa: Effect<R, E, A>,
   f: (e: E) => Effect<R1, E1, boolean>
): Effect<R & R1, E | E1, A> => catchAll_(fa, (e) => chain_(f(e), (b) => (b ? retryWhileM_(fa, f) : fail(e))));

/**
 * Retries this effect while its error satisfies the specified effectful predicate.
 */
export const retryWhileM = <E, R1, E1>(f: (e: E) => Effect<R1, E1, boolean>) => <R, A>(fa: Effect<R, E, A>) =>
   retryWhileM_(fa, f);

/**
 * Retries this effect while its error satisfies the specified predicate.
 */
export const retryWhile_ = <R, E, A>(fa: Effect<R, E, A>, f: (e: E) => boolean) => retryWhileM_(fa, flow(f, pure));

/**
 * Retries this effect while its error satisfies the specified predicate.
 */
export const retryWhile = <E>(f: (e: E) => boolean) => <R, A>(fa: Effect<R, E, A>) => retryWhile_(fa, f);
