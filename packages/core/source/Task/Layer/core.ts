...
export const pure = <T>(has: H.Tag<T>) => (resource: T) =>
   layer<unknown, never, H.Has<T>>(M.chain_(M.fromTask(T.pure(resource)), (a) => environmentFor(has, a)));

export const prepare = <T>(has: H.Tag<T>) => <R, E, A extends T>(acquire: T.Task<R, E, A>) => ({
   open: <S1, R1, E1>(open: (_: A) => T.Task<R1, E1, any>) => ({
...
export const fromTask = <T>(has: H.Tag<T>) => <R, E>(resource: T.Task<R, E, T>) =>
   layer<R, E, H.Has<T>>(M.chain_(M.fromTask(resource), (a) => environmentFor(has, a)));
...
export const fromRawTask = <R, E, A>(resource: T.Task<R, E, A>) => layer<R, E, A>(M.fromTask(resource));

export const both_ = <R, E, A, R2, E2, A2>(left: Layer<R, E, A>, right: Layer<R2, E2, A2>) =>
   layer<R & R2, E | E2, readonly [A, A2]>(
      M.chain_(left.build, (l) => M.chain_(right.build, (r) => M.fromTask(T.total(() => tuple(l, r)))))
   );
...
export const merge_ = <R, E, A, S2, R2, E2, A2>(left: Layer<R, E, A>, right: Layer<R2, E2, A2>) =>
   layer<R & R2, E | E2, A & A2>(
      M.chain_(left.build, (l) => M.chain_(right.build, (r) => M.fromTask(T.total(() => ({ ...l, ...r })))))
   );
