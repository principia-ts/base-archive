...
export const unwrap = <R, E, A>(fa: T.Task<R, E, Stream<R, E, A>>): Stream<R, E, A> => flatten(fromTask(fa));
...
export const fromSchedule: <R, A>(schedule: Sc.Schedule<R, unknown, A>) => Stream<R & HasClock, never, A> = flow(
   Sc.driver,
   T.map((driver) => repeatTaskOption(driver.next(constVoid()))),
   unwrap
);
...
export const unfoldChunkM = <Z>(z: Z) => <R, E, A>(
   f: (z: Z) => T.Task<R, E, Option<readonly [ReadonlyArray<A>, Z]>>
): Stream<R, E, A> =>
   new Stream(
      pipe(
         M.do,
...
export const combineChunks = <R1, E1, B>(that: Stream<R1, E1, B>) => <Z>(z: Z) => <R, E, A, C>(
   ...
   ) => T.Task<R & R1, never, Exit<Option<E | E1>, readonly [ReadonlyArray<C>, Z]>>
 ...
 */
export const ensuringFirst_ = <R, E, A, R1>(self: Stream<R, E, A>, fin: Task<R1, never, unknown>) =>
   new Stream<R & R1, E, A>(M.ensuringFirst_(self.proc, fin));

/**
 * Executes the provided finalizer before this stream's finalizers run.
 */
export const ensuringFirst = <R1>(fin: Task<R1, never, unknown>) => <R, E, A>(self: Stream<R, E, A>) =>
