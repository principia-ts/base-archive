...
export function route_<R, E, R1, E1>(
  routes: Routes<R, E>,
  f: (req: Context, next: T.AIO<R, E, void>) => T.AIO<R1, E1, void>
): Routes<R1, E1> {
...
export function addRoute_<R, E, R1, E1>(
  ...
  f: (ctx: Context) => T.AIO<R1 & Has<Context>, E1, void>
): Routes<R & R1, E | E1> {
  return route_(
    routes,
    (ctx, n): T.AIO<R & R1, E | E1, void> =>
...
export function addRouteM_<R, E, R1, R2, E2>(
  ...
  f: (ctx: Context) => T.AIO<R2 & Has<Context>, E2, void>
): Routes<R & R1 & R2, E | E2> {
  return route_(routes, (ctx, next) =>
    T.chain_(
      path(ctx),
      (b): T.AIO<R & R1 & R2, E | E2, void> => (b ? T.giveService(Context)(ctx)(f(ctx)) : next)
    )
  );
}
