...
export function addRoute_<R, E, R1, E1>(
  ...
  return route_(
    ...
    (ctx, n): IO<R & R1, E | E1, void> =>
      ctx.req.url ? (path(ctx) ? I.giveService(tag)(ctx)(f(ctx)) : n) : n
  );
}
...
export function addRoute<R1, E1>(
  ...
  return (routes) => addRoute_(tag, routes, path, f);
}
...
export function addRouteM_<R, E, R1, R2, E2>(
  ...
  return route_(routes, (ctx, next) =>
    I.flatMap_(
      path(ctx),
      (b): IO<R & R1 & R2, E | E2, void> => (b ? I.giveService(tag)(ctx)(f(ctx)) : next)
    )
  );
}
...
export function addRouteM<R1, R2, E2>(
  ...
  return (routes) => addRouteM_(tag, routes, path, f);
}
