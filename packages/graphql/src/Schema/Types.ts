...
export class GQLField<Root, Args, T, R, E, A> {
  ...
  constructor(
    ...
    readonly resolve: ResolverF<Root, Args, T, R, E, A>
  ) {}
}
...
export class GQLScalar<N extends string, R, I, O> {
  ...
  constructor(
    readonly ast: ScalarTypeDefinitionNode,
    readonly name: N,
    readonly fns: ScalarFunctions<I, O>
  ) {}
}
...
export type FieldRecord<Root, T, F> = Partial<
  {
    [K in keyof Root]: Root[K] extends { [x: string]: any }
      ...
      :
          | GQLScalarField<Root[K]>
          | GQLField<Root, any, T, any, any, Root[K]>
          | GQLObjectField<Root, T, any, any, Root[K]>;
  }
> &
  {
...
export type FieldResolverRecord<Fs extends FieldRecord<any, any, Fs>> = Compute<
  ExcludeMatchingProperties<
    {
      [K in keyof Fs]: Fs[K] extends GQLField<
        infer Root,
        infer Args,
        infer T,
        infer R,
