...
export interface Decoder<F extends HKT.URIS, C, I, O> extends K.KleisliDecoder<F, V<C>, I, DecodeErrors, O> {
  readonly _meta: {
...
export function fromRefinement<M extends HKT.URIS, C>(
  M: P.MonadFail<M, V<C>>
): <I, A extends I>(refinement: Refinement<I, A>, expected: string, info?: ErrorInfo) => Decoder<M, C, I, A> {
...
export function mapLeftWithInput_<M extends HKT.URIS, C>(
  M: P.Bifunctor<M, C>
): <I, A>(decoder: Decoder<M, C, I, A>, f: (input: I, e: DecodeErrors) => DecodeErrors) => Decoder<M, C, I, A> {
  return (decoder, f) => ({
...
export function mapLeftWithInput<M extends HKT.URIS, C>(
  M: P.Bifunctor<M, C>
): <I>(f: (input: I, e: DecodeErrors) => DecodeErrors) => <A>(decoder: Decoder<M, C, I, A>) => Decoder<M, C, I, A> {
  return (f) => (decoder) => mapLeftWithInput_(M)(decoder, f)
}

export function withMessage<M extends HKT.URIS, C>(
  M: P.Bifunctor<M, C>
): <I>(message: (input: I, e: DecodeErrors) => string) => <A>(decoder: Decoder<M, C, I, A>) => Decoder<M, C, I, A> {
  return (message) => mapLeftWithInput(M)((input, e) => FS.element(DE.wrap({ message: message(input, e) }, e)))
}

export function wrapInfo<M extends HKT.URIS, C>(
  M: P.Bifunctor<M, C>
): (info: ErrorInfo | undefined) => <I, A>(decoder: Decoder<M, C, I, A>) => Decoder<M, C, I, A> {
  return (info) => (decoder) => (info ? mapLeftWithInput_(M)(decoder, (_, e) => FS.element(DE.wrap(info, e))) : decoder)
}
...
): (info?: ErrorInfo) => <I, A>(or: Decoder<M, C, I, A>) => Decoder<M, C, I | null | undefined, A | null> {
  return (info) => (or) => ({
...
export function optional<M extends HKT.URIS, C>(
  M: P.Applicative<M, V<C>> & P.Bifunctor<M, C>
): (info?: ErrorInfo) => <I, A>(or: Decoder<M, C, I, A>) => Decoder<M, C, I | null | undefined, O.Option<A>> {
  return (info) => (or) => ({
...
export function type<M extends HKT.URIS, C>(
  ...
  return (properties, info) => compose_(M)(UnknownRecord(M)(info) as any, fromType(M)(properties, info))
}
...
export function fromPartial<M extends HKT.URIS, C>(
  ...
  properties: P,
  info?: ErrorInfo
) => Decoder<M, C, Partial<{ [K in keyof P]: InputOf<M, P[K]> }>, Partial<{ [K in keyof P]: TypeOf<M, P[K]> }>> {
  return (properties, info) => {
    const name: string = pipe(
      properties,
      R.foldLeftWithIndex([] as string[], (b, k, a) => [...b, `${k}?: ${a._meta.name}`]),
...
export function partial<M extends HKT.URIS, C>(
  ...
  >
> {
  return (properties, info) => compose_(M)(UnknownRecord(M)(info) as any, fromPartial(M)(properties, info))
}

export function fromArray<M extends HKT.URIS, C>(
  M: P.Applicative<M, V<C>> & P.Bifunctor<M, C>
): <I, A>(item: Decoder<M, C, I, A>, info?: ErrorInfo) => Decoder<M, C, ReadonlyArray<I>, ReadonlyArray<A>> {
  return (item, info) => {
...
export function array<M extends HKT.URIS, C>(
  M: P.MonadFail<M, V<C>> & P.Applicative<M, V<C>> & P.Bifunctor<M, C>
): <A>(item: Decoder<M, C, unknown, A>, info?: ErrorInfo) => Decoder<M, C, unknown, ReadonlyArray<A>> {
...
export function record<M extends HKT.URIS, C>(
  M: P.MonadFail<M, V<C>> & P.Applicative<M, V<C>> & P.Bifunctor<M, C>
): <A>(codomain: Decoder<M, C, unknown, A>, info?: ErrorInfo) => Decoder<M, C, unknown, R.ReadonlyRecord<string, A>> {
...
export function fromTuple<M extends HKT.URIS, C>(
  ...
  info?: ErrorInfo
) => Decoder<M, C, [...{ [K in keyof A]: InputOf<M, A[K]> }], [...{ [K in keyof A]: TypeOf<M, A[K]> }]> {
...
export function tuple<M extends HKT.URIS, C>(
  ...
  return (...components) => (info) => compose_(M)(UnknownArray(M)(info) as any, fromTuple(M)(...components)(info))
}
...
export function intersectAll<M extends HKT.URIS, C>(
  M: P.Applicative<M, V<C>> & P.Bifunctor<M, C>
): <A extends readonly [Decoder<M, C, any, any>, Decoder<M, C, any, any>, ...(readonly Decoder<M, C, any, any>[])]>(
  decoders: A,
  info?: ErrorInfo
) => Decoder<
  M,
...
export function sum_<M extends HKT.URIS, C>(
  ...
  return (tag, members, info) => compose_(M)(UnknownRecord(M)(info) as any, fromSum_(M)(tag, members, info) as any)
}
...
export function sum<M extends HKT.URIS, C>(
  ...
  tag: T,
  info?: ErrorInfo
) => <P extends Record<string, Decoder<M, C, any, any>>>(members: P) => Decoder<M, C, unknown, TypeOf<M, P[keyof P]>> {
