...
export function fromGuard<I, A extends I>(guard: Guard<I, A>, expected: string, info?: ErrorInfo): EitherDecoder<I, A> {
...
export function compose_<I, A, B>(from: EitherDecoder<I, A>, to: EitherDecoder<A, B>): EitherDecoder<I, B> {
  return {
...
export function compose<A, B>(to: EitherDecoder<A, B>): <I>(from: EitherDecoder<I, A>) => EitherDecoder<I, B> {
...
export function alt_<I, A>(me: EitherDecoder<I, A>, that: () => EitherDecoder<I, A>): EitherDecoder<I, A> {
  return {
...
export function alt<I, A>(that: () => EitherDecoder<I, A>): (me: EitherDecoder<I, A>) => EitherDecoder<I, A> {
...
export function wrapInfo<I>(info?: ErrorInfo): <A>(decoder: EitherDecoder<I, A>) => EitherDecoder<I, A> {
...
export function array<A>(item: EitherDecoder<unknown, A>, info?: ErrorInfo): EitherDecoder<unknown, ReadonlyArray<A>> {
...
export function intersectAll<
  A extends readonly [EitherDecoder<any, any>, EitherDecoder<any, any>, ...(readonly EitherDecoder<any, any>[])]
>(
  decoders: A,
  name?: string
): EitherDecoder<
...
> {
  ...
  const decode = A.foldLeft_(rest, K.intersect_(M)(left, right), (b, a) => K.intersect_(M)(b, a)).decode
...
export function sum<T extends string>(
  tag: T,
  info?: ErrorInfo
): <A>(members: { [K in keyof A]: EitherDecoder<unknown, A[K]> }) => EitherDecoder<unknown, A[keyof A]> {
  return (members) => sum_(tag, members, info)
}

export function lazy<I, A>(id: string, f: () => EitherDecoder<I, A>, info?: ErrorInfo): EitherDecoder<I, A> {
  return pipe(
    {
