  ...
  return {
    before(ctx: ts.TransformationContext) {
      ...
      return (sourceFile: ts.SourceFile) => {
        ...
        const isModule = sourceFile.statements.find((s) => /(import|export)/.test(s.getText())) != null
        ...
        function visitor(node: ts.Node): ts.VisitResult<ts.Node> {
          ...
          const isTracing = tracingOn && checkRegionAt(regions, nodeStart.line, nodeStart.character)
        ...
        const fileNode = factory.createVariableStatement(
          ...
          factory.createVariableDeclarationList(
            [factory.createVariableDeclaration(fileVar, undefined, undefined, factory.createStringLiteral(finalName))],
            ts.NodeFlags.Const
          )
        )
        ...
        if (tracingOn) {
          ...
          return factory.updateSourceFile(visited, [
            factory.createImportDeclaration(
              ...
              factory.createStringLiteral('@principia/compile/util')
            ),
            fileNode,
            ...visited.statements
          ])
