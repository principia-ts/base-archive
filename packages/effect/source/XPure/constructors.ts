...
export const concrete = <S1, S2, R, E, A>(_: XPure<S1, S2, R, E, A>): Concrete<S1, S2, R, E, A> =>
   _ as any;
...
export const succeed = <A, S1 = unknown, S2 = never>(a: A): XPure<S1, S2, unknown, never, A> =>
   new PureInstruction(a);

export const total = <A, S1 = unknown, S2 = never>(
   thunk: Lazy<A>
): XPure<S1, S2, unknown, never, A> => new TotalInstruction(thunk);
...
): XPure<S1, S2, unknown, never, A> => new ModifyInstruction(f);
...
export const _partial = <A, E>(
   f: () => A,
   onThrow: (reason: unknown) => E
): XPure<unknown, never, unknown, E, A> => new PartialInstruction(f, onThrow);

export const partial = <E>(onThrow: (reason: unknown) => E) => <A>(f: () => A) =>
   _partial(f, onThrow);
