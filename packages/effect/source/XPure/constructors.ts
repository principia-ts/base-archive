 ...
 * ```
 */
export const succeed = <A, S1 = unknown, S2 = never>(a: A): XPure<S1, S2, unknown, never, A> => PureInstruction(a);

export const total = <A, S1 = unknown, S2 = never>(thunk: Lazy<A>): XPure<S1, S2, unknown, never, A> =>
   TotalInstruction(thunk);

export const fail = <E>(e: E): XPure<unknown, never, unknown, E, never> => FailInstruction(e);
