 ...
 * Similarly, embedded inside every `Stream` is an Effect program: `Effect<X, R, Option<E>, ReadonlyArray<A>>`.
...
export class Stream<R, E, A> {
   ...
   constructor(readonly proc: M.Managed<R, never, T.Effect<R, Option<E>, ReadonlyArray<A>>>) {}
}
...
export class Chain<R_, E_, O, O2> {
   constructor(
      ...
      readonly outerStream: T.Effect<R_, Option<E_>, ReadonlyArray<O>>,
      readonly currOuterChunk: XR.Ref<[ReadonlyArray<O>, number]>,
      readonly currInnerStream: XR.Ref<T.Effect<R_, Option<E_>, ReadonlyArray<O2>>>,
   ...
   pullNonEmpty<R, E, O>(pull: T.Effect<R, Option<E>, ReadonlyArray<O>>): T.Effect<R, Option<E>, ReadonlyArray<O>> {
      return pipe(
         pull,
   ...
   pullOuter() {
      return pipe(
         this.currOuterChunk,
         XR.modify(([chunk, nextIdx]): [T.Effect<R_, Option<E_>, O>, [ReadonlyArray<O>, number]] => {
   ...
   apply(): T.Effect<R_, Option<E_>, ReadonlyArray<O2>> {
      return pipe(
