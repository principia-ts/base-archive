...
import { accessServiceM, replaceService } from "../Effect/functions/service";
...
export class LiveRandom extends Random {
   ...
   nextBoolean: T.UIO<boolean> = T._chain(this.next, (n) => T.total(() => n > 0.5));
   ...
   nextRange: (low: number, high: number) => T.UIO<number> = (low, high) =>
      T._chain(this.next, (n) => T.total(() => (high - low) * n + low));

   nextIntBetween: (low: number, high: number) => T.UIO<number> = (low, high) =>
      T._chain(this.next, (n) => T.total(() => Math.floor((high - low + 1) * n + low)));
...
export const setSeed = (seed: string) => accessServiceM(HasRandom)((_) => _.setSeed(seed));
