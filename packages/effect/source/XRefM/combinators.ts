...
import { left, right } from "@principia/core/Either";
 ...
 * `update`.
 */
export const modify = <R1, E1, B, A>(f: (a: A) => T.Effect<R1, E1, readonly [B, A]>) => <RA, RB, EA, EB>(
   self: XRefM<RA, RB, EA, EB, A, A>
 ...
 * value immediately before modification.
 */
export const getAndUpdateSome = <R1, E1, A>(f: (a: A) => Option<T.Effect<R1, E1, A>>) => <RA, RB, EA, EB>(
   self: XRefM<RA, RB, EA, EB, A, A>
...
export const modifySome_ = <RA, RB, EA, EB, R1, E1, A, B>(
   self: XRefM<RA, RB, EA, EB, A, A>,
   def: B,
   f: (a: A) => Option<T.Effect<R1, E1, readonly [B, A]>>
) =>
   pipe(
      self,
      modify((v) =>
 ...
 * This is a more powerful version of `updateSome`.
 */
export const modifySome = <B>(def: B) => <R1, E1, A>(f: (a: A) => Option<T.Effect<R1, E1, [B, A]>>) => <RA, RB, EA, EB>(
   self: XRefM<RA, RB, EA, EB, A, A>
 ...
 * Atomically modifies the `RefM` with the specified function.
 */
export const updateSome = <R1, E1, A>(f: (a: A) => Option<T.Effect<R1, E1, A>>) => <RA, RB, EA, EB>(
   self: XRefM<RA, RB, EA, EB, A, A>
 ...
 * Atomically modifies the `RefM` with the specified function.
 */
export const updateSomeAndGet = <R1, E1, A>(f: (a: A) => Option<T.Effect<R1, E1, A>>) => <RA, RB, EA, EB>(
   self: XRefM<RA, RB, EA, EB, A, A>
 ...
 * fails with `None`.
 */
export const collectM = <B, RC, EC, C>(f: (b: B) => Option<T.Effect<RC, EC, C>>) => <RA, RB, EA, EB, A>(
   self: XRefM<RA, RB, EA, EB, A, B>
 ...
 * predicate is satisfied or else fails with `None`.
 */
export const filterInputM = <A, RC, EC, A1 extends A = A>(f: (a: A1) => T.Effect<RC, EC, boolean>) => <
   RA,
   RB,
   EA,
   EB,
 ...
 * satisfied or else fails with `None`.
 */
export const filterOutputM = <B, RC, EC>(f: (b: B) => T.Effect<RC, EC, boolean>) => <RA, RB, EA, EB, A>(
   self: XRefM<RA, RB, EA, EB, A, B>
 ...
 * `XRefM`.
 */
export const tapInput = <A, RC, EC, A1 extends A = A>(f: (a: A1) => T.Effect<RC, EC, any>) => <RA, RB, EA, EB, B>(
   self: XRefM<RA, RB, EA, EB, A, B>
) => tapInput_(self, f);
