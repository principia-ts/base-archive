...
import type { Runtime } from "../Effect/functions/runtime";
import { makeRuntime } from "../Effect/functions/runtime";
...
export const region = <K, T>(h: Tag<Region<T, K>>) => <R, E>(_: L.Layer<R, E, T>): L.Layer<R, E, Has<Region<T, K>>> =>
   pipe(L.fromRawEffect(T.asks((r: T): Has<Region<T, K>> => ({ [h.key]: r } as any))), L.consuming(_));
