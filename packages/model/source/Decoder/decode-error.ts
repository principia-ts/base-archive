...
import type { Tree } from "@principia/core/Tree";
import { drawTree } from "@principia/core/Tree";
...
export const error = (actual: unknown, expected: string, info?: ErrorInfo): DecodeError =>
   isInfoPopulated(info)
      ? FS.combine(FS.of(DE.leaf(actual, expected)), FS.of(DE.info(info)))
      : FS.of(DE.leaf(actual, expected));
