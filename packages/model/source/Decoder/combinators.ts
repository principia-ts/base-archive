...
export const wrapInfo = <I>(info?: ErrorInfo) => <A>(decoder: Decoder<I, A>): Decoder<I, A> =>
   info ? mapLeftWithInput_(decoder, (_, e) => FS.of(DE.wrap(info, e))) : decoder;

export const withMessage = <I>(
   message: (input: I, e: DecodeError) => string
): (<A>(decoder: Decoder<I, A>) => Decoder<I, A>) =>
   mapLeftWithInput((input, e) => FS.of(DE.wrap({ message: message(input, e) }, e)));
...
export const nullable: (info?: ErrorInfo) => <I, A>(or: Decoder<I, A>) => Decoder<undefined | null | I, null | A> = (
   ...
   decode: K.nullable_(M)(or, (u, e) =>
      FS.combine(FS.of(DE.member(0, error(u, "null | undefined", info))), FS.of(DE.member(1, e)))
   ).decode,
   _meta: {
...
export const fromType = <P extends Record<string, Decoder<any, any>>>(
   ...
   return pipe(
      {
         decode: K.fromType_(M)(properties, (k, e) => FS.of(DE.key(k, DE.required, e))).decode,
         _meta: {
            name
         }
      },
...
export const fromPartial = <P extends Record<string, Decoder<any, any>>>(
   ...
   return pipe(
      {
         decode: K.fromPartial_(M)(properties, (k, e) => FS.of(DE.key(k, DE.optional, e))).decode,
         _meta: {
            name
         }
      },
...
export const fromArray = <I, A>(item: Decoder<I, A>, info?: ErrorInfo): Decoder<ReadonlyArray<I>, ReadonlyArray<A>> => {
   ...
   return pipe(
      {
         decode: K.fromArray_(M)(item, (i, e) => FS.of(DE.index(i, DE.optional, e))).decode,
         _meta: { name }
      },
      wrapInfo({ name, ...info })
   );
...
export const fromRecord = <I, A>(
   ...
   return pipe(
      {
         decode: K.fromRecord_(M)(codomain, (k, e) => FS.of(DE.key(k, DE.optional, e))).decode,
         _meta: { name }
      },
      wrapInfo({ name, ...info })
   );
...
export const fromTuple = <C extends ReadonlyArray<Decoder<any, any>>>(
   ...
   return pipe(
      {
         decode: K.fromTuple(M)((i, e) => FS.of(DE.index(i, DE.required, e)))(...components).decode,
         _meta: { name }
      },
      wrapInfo({ name, ...info })
   );
...
export const union = (info?: ErrorInfo) => <MS extends readonly [Decoder<any, any>, ...Array<Decoder<any, any>>]>(
   ...
   return pipe(
      {
         decode: K.union(M)((i, e) => FS.of(DE.member(i, e)))(...members).decode,
         _meta: { name }
...
export const lazy = <I, A>(id: string, f: () => Decoder<I, A>, info?: ErrorInfo): Decoder<I, A> =>
   pipe(
      {
         decode: K.lazy_(M)(id, f, (id, e) => FS.of(DE.lazy(id, e))).decode,
         _meta: {
            name: info?.name ?? id
         }
      },
