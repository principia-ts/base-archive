...
export const PrimitivesGuard = implementInterpreter<G.URI, Alg.PrimitivesURI>()((_) => ({
  ...
  literal: (...values) => (config) => (env) =>
    applyGuardConfig(config?.config)(G.literal(...values), env, {}),
  stringLiteral: (value, config) => (env) =>
    applyGuardConfig(config?.config)(G.literal(value), env, {}),
  numberLiteral: (value, config) => (env) =>
    applyGuardConfig(config?.config)(G.literal(value), env, {}),
  bigint: (config) => (env) =>
    applyGuardConfig(config?.config)({ is: (u): u is bigint => typeof u === "bigint" }, env, {}),
  date: (config) => (env) =>
    applyGuardConfig(config?.config)({ is: (u): u is Date => u instanceof Date }, env, {}),
