...
import * as Enc from "@principia/decoders/Encoder";
...
export const SumEncoder = implementInterpreter<Enc.URI, Alg.SumURI>()((_) => ({
  taggedUnion: (tag, types, config) => (env) =>
    pipe(
      ...
      R.map((f) => f(env)),
      (encoders) =>
        applyEncoderConfig(config?.config)(Enc.sum(tag)(encoders) as any, env, encoders as any)
    ),
  ...
  option: (a, config) => (env) =>
    pipe(a(env), (encoder) =>
      applyEncoderConfig(config?.config)({ encode: O.map(encoder.encode) }, env, encoder)
    )
}));
