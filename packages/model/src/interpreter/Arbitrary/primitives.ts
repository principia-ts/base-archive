...
export const PrimitivesArbitrary = implementInterpreter<ArbURI, Alg.PrimitivesURI>()((_) => ({
  string: (config) => (env) =>
    applyArbitraryConfig(config?.config)(accessFastCheck(env).string(), env, {}),
  number: (config) => (env) =>
    applyArbitraryConfig(config?.config)(accessFastCheck(env).float(), env, {}),
  boolean: (config) => (env) =>
    applyArbitraryConfig(config?.config)(accessFastCheck(env).boolean(), env, {}),
  ...
  numberLiteral: (value, config) => (env) =>
    applyArbitraryConfig(config?.config)(accessFastCheck(env).constant(value), env, {}),
  bigint: (config) => (env) =>
    applyArbitraryConfig(config?.config)(accessFastCheck(env).bigInt(), env, {}),
  date: (config) => (env) =>
    applyArbitraryConfig(config?.config)(accessFastCheck(env).date(), env, {}),
  array: (item, config) => (env) =>
    pipe(item(env), (arb) =>
      applyArbitraryConfig(config?.config)(accessFastCheck(env).array(arb), env, arb)
    ),
  nonEmptyArray: (item, config) => (env) =>
    pipe(item(env), (arb) =>
      applyArbitraryConfig(config?.config)(
        accessFastCheck(env).array(arb).filter(isNonEmpty) as any,
        env,
        arb
      )
    ),
  ...
  UUID: (config) => (env) =>
    applyArbitraryConfig(config?.config)(accessFastCheck(env).uuid() as any, env, {})
}));
