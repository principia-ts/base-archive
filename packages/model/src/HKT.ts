...
export type InterpretedKind<
  ...
  E,
  A
> = URI extends InterpreterURIS ? URItoInterpreted<Env, S, R, E, A>[URI] : never;
...
export type Algebra<
  ...
  IURI extends InterpreterURIS,
  Env extends AnyEnv
> = UnionToIntersection<URItoAlgebra<IURI, Env>[AURI]>;
...
export const interpretable = <T extends { [_overloads]?: any }>(program: T): Overloads<T> =>
  program as Overloads<T>;

export type InferredAlgebra<PURI extends ProgramURIS, Env extends AnyEnv> = Algebra<
  URItoAURIS[PURI],
  UIHKT,
  Env
>;
...
export interface InferredProgram<PURI extends ProgramURIS, Env extends AnyEnv, S, R, E, A> {
  ...
  [_overloads]?: {
    <F extends Exclude<InterpreterURIS, UIHKT>>(
      a: Algebra<URItoAURIS[PURI], F, Env>
    ): InterpretedKind<F, { [K in F & keyof Env]: Env[K] }, S, R, E, A>;
  };
}
...
export interface ProgramInterpreter<PURI extends ProgramURIS, RURI extends ResultURIS> {
  <Env extends AnyEnv, S, R, E, A>(program: URItoProgram<Env, S, R, E, A>[PURI]): URItoResult<
    S,
    R,
    E,
    A
...
const inhabitInterpreterAndAlgebra = <PURI extends ProgramURIS, RURI extends ResultURIS, T>(
  t: T
): T & InhabitedInterpreterAndAlgebra<PURI, RURI> =>
  t as T & InhabitedInterpreterAndAlgebra<PURI, RURI>;
...
export type TaggedUnionConfigKind<CURI extends ConfigURIS, Types> = {
  [k in keyof Types]: Types[k] extends InterpretedHKT<
    infer U,
    infer Env,
    infer S,
    infer R,
...
export type InterfaceConfigKind<CURI extends ConfigURIS, Props> = {
  [k in keyof Props]: Props[k] extends InterpretedHKT<
    infer U,
    infer Env,
    infer S,
    infer R,
