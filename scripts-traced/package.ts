...
import { onLeft, onRight, readFile, writeFile } from '../scripts/common'
...
pipe(
  ...
  TE.map((content: any) => {
    ...
    const exports     = pipe(
      ...
      R.map((ex: any) => {
        ...
        if (ex['traced']) {
          mut_ex['traced'] = R.Functor.map(ex['traced'], (ex1: any) => {
            const mut_ex1     = { ...ex1 }
            let esm           = (ex1['import'] as string).split('/')
            esm               = [...esm.slice(0, 1), ...esm.slice(2)]
            mut_ex1['import'] = esm.join('/')

            let cjs            = (ex1['require'] as string).split('/')
            cjs                = [...cjs.slice(0, 1), ...cjs.slice(2)]
            mut_ex1['require'] = cjs.join('/')
  ...
  TE.chain(() => writeFile(Path.resolve(process.cwd(), 'dist/dist-traced/cjs/package.json'), cjsJSON)),
  TE.chain(() => writeFile(Path.resolve(process.cwd(), 'dist/dist-traced/esm-shake/package.json'), shakeJSON)),
  TE.chain(() => writeFile(Path.resolve(process.cwd(), 'dist/dist-traced/esm-fix/package.json'), esmJSON)),
